# ECCE modernization session history (archived 2026-09-02)

This file is the verbatim former content of the repo's `CLAUDE.md`, preserved
here when `CLAUDE.md` was rebuilt from a session-by-session bug-fix diary
into a lean, architecture-focused briefing (see the current `CLAUDE.md` for
that). Nothing below has been edited or re-verified against the current
state of the code — treat it as a historical record of investigations, root
causes, and fixes applied during the wx2.8→3.2 / C++14→17 / build_ecce→CMake
/ Python2→3 modernization effort, not as a live status document. Where a
later entry in this same file corrects or supersedes an earlier one, both
are kept, in original order, exactly as they were — read chronologically,
later entries win.

For a shorter map of where things live in the codebase today, see
`CLAUDE.md`. For the earliest chapter of this same effort (the initial
wx port completion, JMS gateway bring-up, and the Python 2→3 syntax pass),
see `ECCE_modernization_status.md` in the repo root, which predates and is
not duplicated by this file. The canonical, most up-to-date narrative
(when accessible) is the claude.ai Project "ECCE" doc,
`claude/ecce-modernization-status.md`.
...
## `scripts/parsers` packaging gap — real fix, but NOT the basis-set error's cause (2026-08-29)
