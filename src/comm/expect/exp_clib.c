 ...
 *
 * RCS: @(#) $Id: exp_clib.c 22364 2012-03-07 21:00:09Z ecce_cm $
 */
...
{
  if (exp_is_debugging) {
    /* GDB 3/3/12  Added 0 arg to suppress compiler warnings */
    fprintf(stderr,str,0);
    if (exp_logfile) fprintf(exp_logfile,str,0);
  }
}
...
/* the key internal variables that this function depends on are:
	...
	exp_match_end
*/
/* GDB 2/3/12 ECCE password filtering */
static char* elide = NULL;
...
#else
                /* GDB 2/3/12 ECCE password filtering
                   Originally developed 7/22/02 */
                if (exp_loguser) {
                  char* passptr = exp_buffer + old_length;
                  /* append an end-of-string so string calls don't bomb */
                  *(passptr + cc) = '\0';
                  char* cptr;
                  if (elide!=NULL && (cptr = strstr(passptr, elide))!=NULL) {
                    fwrite(passptr,1,cptr-passptr,stdout);
                    printf("(elided)");
                    cptr += strlen(elide);
                    if (*cptr != '\0')
                      printf("%s", cptr);
                  } else
                    fwrite(passptr,1,cc,stdout);
                }
#endif
...
/* GDB 2/3/12 ECCE password filtering function
   Originally developed 7/22/02 */
void
exp_elide(s)
const char *s;
