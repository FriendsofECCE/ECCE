///////////////////////////////////////////////////////////////////////////////
// Name:        src/ewxaui/framemanager.cpp
...
WX_CHECK_BUILD_OPTIONS("ewxAUI")

#include "wx/arrimpl.cpp"
WX_DECLARE_OBJARRAY(wxRect, ewxAuiRectArray);
WX_DEFINE_OBJARRAY(ewxAuiRectArray)
WX_DEFINE_OBJARRAY(ewxAuiDockUIPartArray)
WX_DEFINE_OBJARRAY(ewxAuiDockInfoArray)
WX_DEFINE_OBJARRAY(ewxAuiPaneButtonArray)
WX_DEFINE_OBJARRAY(ewxAuiPaneInfoArray)

ewxAuiPaneInfo ewxAuiNullPaneInfo;
ewxAuiDockInfo ewxAuiNullDockInfo;
DEFINE_EVENT_TYPE(wxEVT_EWXAUI_PANE_BUTTON)
DEFINE_EVENT_TYPE(wxEVT_EWXAUI_PANE_CLOSE)
DEFINE_EVENT_TYPE(wxEVT_EWXAUI_PANE_MAXIMIZE)
DEFINE_EVENT_TYPE(wxEVT_EWXAUI_PANE_RESTORE)
DEFINE_EVENT_TYPE(wxEVT_EWXAUI_RENDER)
DEFINE_EVENT_TYPE(wxEVT_EWXAUI_FIND_MANAGER)
...
IMPLEMENT_DYNAMIC_CLASS(ewxAuiManagerEvent, wxEvent)
IMPLEMENT_CLASS(ewxAuiManager, wxEvtHandler)
...
class ewxPseudoTransparentFrame : public wxFrame
{
public:
    ewxPseudoTransparentFrame(wxWindow* parent = NULL,
...
private:
    ...
    DECLARE_DYNAMIC_CLASS(ewxPseudoTransparentFrame)
    DECLARE_EVENT_TABLE()
};


IMPLEMENT_DYNAMIC_CLASS(ewxPseudoTransparentFrame, wxFrame)

BEGIN_EVENT_TABLE(ewxPseudoTransparentFrame, wxFrame)
    EVT_PAINT(ewxPseudoTransparentFrame::OnPaint)
    EVT_SIZE(ewxPseudoTransparentFrame::OnSize)
#ifdef __WXGTK__
    EVT_WINDOW_CREATE(ewxPseudoTransparentFrame::OnWindowCreate)
#endif
END_EVENT_TABLE()
...
// the dock and pane info.  ewxAuiDockInfo's usually contain pointers
// to ewxAuiPaneInfo classes, thus this function is necessary to reliably
// reconstruct that relationship in the new dock info and pane info arrays

static void CopyDocksAndPanes(ewxAuiDockInfoArray& dest_docks,
                              ewxAuiPaneInfoArray& dest_panes,
                              const ewxAuiDockInfoArray& src_docks,
                              const ewxAuiPaneInfoArray& src_panes)
{
    ...
    {
        ewxAuiDockInfo& dock = dest_docks.Item(i);
...
static int GetMaxLayer(const ewxAuiDockInfoArray& docks,
                       int dock_direction)
{
    int i, dock_count, max_layer = 0;
    for (i = 0, dock_count = docks.GetCount(); i < dock_count; ++i)
    {
        ewxAuiDockInfo& dock = docks.Item(i);
...
// the highest layer inside the specified dock
static int GetMaxRow(const ewxAuiPaneInfoArray& panes, int direction, int layer)
{
...
static void DoInsertDockLayer(ewxAuiPaneInfoArray& panes,
                              int dock_direction,
                              int dock_layer)
{
    int i, pane_count;
...
static void DoInsertDockRow(ewxAuiPaneInfoArray& panes,
                            int dock_direction,
                            int dock_layer,
                            int dock_row)
{
...
static void DoInsertPane(ewxAuiPaneInfoArray& panes,
                         int dock_direction,
                         int dock_layer,
                         int dock_row,
                         int dock_pos)
...
static void FindDocks(ewxAuiDockInfoArray& docks,
                      int dock_direction,
                      int dock_layer,
                      int dock_row,
                      ewxAuiDockInfoPtrArray& arr)
{
...
// If found, the corresponding ewxAuiPaneInfo pointer is returned, otherwise NULL.
static ewxAuiPaneInfo* FindPaneInDock(const ewxAuiDockInfo& dock, wxWindow* window)
{
    int i, count = dock.panes.GetCount();
    for (i = 0; i < count; ++i)
    {
        ewxAuiPaneInfo* p = dock.panes.Item(i);
        if (p->window == window)
            return p;
    }
    return NULL;
...
static void RemovePaneFromDocks(ewxAuiDockInfoArray& docks,
                                ewxAuiPaneInfo& pane,
                                ewxAuiDockInfo* ex_cept  = NULL  )
{
    int i, dock_count;
    for (i = 0, dock_count = docks.GetCount(); i < dock_count; ++i)
    {
        ...
        if (&d == ex_cept)
            continue;
        ewxAuiPaneInfo* pi = FindPaneInDock(d, pane.window);
        if (pi)
            d.panes.Remove(pi);
    }
}
...
static void RenumberDockRows(ewxAuiDockInfoPtrArray& docks)
{
    int i, dock_count;
    for (i = 0, dock_count = docks.GetCount(); i < dock_count; ++i)
    {
...
// are turned off
static void SetActivePane(ewxAuiPaneInfoArray& panes, wxWindow* active_pane)
{
    int i, pane_count;
    for (i = 0, pane_count = panes.GetCount(); i < pane_count; ++i)
    {
        ewxAuiPaneInfo& pane = panes.Item(i);
        pane.state &= ~ewxAuiPaneInfo::optionActive;
...
static int PaneSortFunc(ewxAuiPaneInfo** p1, ewxAuiPaneInfo** p2)
{
    return ((*p1)->dock_pos < (*p2)->dock_pos) ? -1 : 1;
}


// -- ewxAuiManager class implementation --


BEGIN_EVENT_TABLE(ewxAuiManager, wxEvtHandler)
    EVT_EWXAUI_PANE_BUTTON(ewxAuiManager::OnPaneButton)
    EVT_EWXAUI_RENDER(ewxAuiManager::OnRender)
    EVT_PAINT(ewxAuiManager::OnPaint)
    EVT_ERASE_BACKGROUND(ewxAuiManager::OnEraseBackground)
    EVT_SIZE(ewxAuiManager::OnSize)
    EVT_SET_CURSOR(ewxAuiManager::OnSetCursor)
    EVT_LEFT_DOWN(ewxAuiManager::OnLeftDown)
    EVT_LEFT_UP(ewxAuiManager::OnLeftUp)
    EVT_MOTION(ewxAuiManager::OnMotion)
    EVT_LEAVE_WINDOW(ewxAuiManager::OnLeaveWindow)
    EVT_CHILD_FOCUS(ewxAuiManager::OnChildFocus)
    EVT_EWXAUI_FIND_MANAGER(ewxAuiManager::OnFindManager)
    EVT_TIMER(101, ewxAuiManager::OnHintFadeTimer)
END_EVENT_TABLE()


ewxAuiManager::ewxAuiManager(wxWindow* managed_wnd, unsigned int flags)
{
    m_action = actionNone;
    m_action_window = NULL;
    m_last_mouse_move = wxPoint();
    m_hover_button = NULL;
    m_art = new ewxAuiDefaultDockArt;
...
ewxAuiManager::~ewxAuiManager()
{
    ...
    {
        ewxAuiPaneInfo& pinfo = m_panes[i];
...
ewxAuiFloatingFrame* ewxAuiManager::CreateFloatingFrame(wxWindow* parent,
                                                      const ewxAuiPaneInfo& pane_info)
{
    return new ewxAuiFloatingFrame(parent, this, pane_info);
}

// GetPane() looks up a ewxAuiPaneInfo structure based
// on the supplied window pointer.  Upon failure, GetPane()
// returns an empty ewxAuiPaneInfo, a condition which can be checked
// by calling ewxAuiPaneInfo::IsOk().
//
// The pane info's structure may then be modified.  Once a pane's
// info is modified, ewxAuiManager::Update() must be called to
// realize the changes in the UI.

ewxAuiPaneInfo& ewxAuiManager::GetPane(wxWindow* window)
{
    int i, pane_count;
...
ewxAuiPaneInfoArray& ewxAuiManager::GetAllPanes()
{
    return m_panes;
}
...
ewxAuiDockUIPart* ewxAuiManager::HitTest(int x, int y)
{
    ewxAuiDockUIPart* result = NULL;

    int i, part_count;
    for (i = 0, part_count = m_uiparts.GetCount(); i < part_count; ++i)
    {
        ewxAuiDockUIPart* item = &m_uiparts.Item(i);
        ...
        if (item->type == ewxAuiDockUIPart::typeDock)
            continue;
        ...
        if ((item->type == ewxAuiDockUIPart::typePane ||
            item->type == ewxAuiDockUIPart::typePaneBorder) && result)
            continue;
...
// options which are global to ewxAuiManager
void ewxAuiManager::SetFlags(unsigned int flags)
{
    // find out if we have to call UpdateHintWindowConfig()
    bool update_hint_wnd = false;
    unsigned int hint_mask = ewxAUI_MGR_TRANSPARENT_HINT |
                             ewxAUI_MGR_VENETIAN_BLINDS_HINT |
                             ewxAUI_MGR_RECTANGLE_HINT;
...
unsigned int ewxAuiManager::GetFlags() const
{
    return m_flags;
}

// Convenience function
bool ewxAuiManager_HasLiveResize(ewxAuiManager& manager)
{
    ...
    return true;
#else
    return (manager.GetFlags() & ewxAUI_MGR_LIVE_RESIZE) == ewxAUI_MGR_LIVE_RESIZE;
#endif
}

// don't use these anymore as they are deprecated
// use Set/GetManagedFrame() instead
void ewxAuiManager::SetFrame(wxFrame* frame)
{
    SetManagedWindow((wxWindow*)frame);
}

wxFrame* ewxAuiManager::GetFrame() const
{
...
// managed by ewxAuiManager.  The |window| parameter does not
// need to be managed by the manager itself.
ewxAuiManager* ewxAuiManager::GetManager(wxWindow* window)
{
    ewxAuiManagerEvent evt(wxEVT_EWXAUI_FIND_MANAGER);
    evt.SetManager(NULL);
...
void ewxAuiManager::UpdateHintWindowConfig()
{
    ...
    if ((m_flags & ewxAUI_MGR_TRANSPARENT_HINT) && can_do_transparent)
    {
    ...
    {
        if ((m_flags & ewxAUI_MGR_TRANSPARENT_HINT) != 0 ||
            (m_flags & ewxAUI_MGR_VENETIAN_BLINDS_HINT) != 0)
        {
            // system can't support transparent fade, or the venetian
            // blinds effect was explicitly requested
            m_hint_wnd = new ewxPseudoTransparentFrame(m_frame,
                                                      wxID_ANY,
                                                      wxEmptyString,
                                                      wxDefaultPosition,
                                                      wxSize(1,1),
...
void ewxAuiManager::SetManagedWindow(wxWindow* wnd)
{
    ...
    if (m_frame->IsKindOf(CLASSINFO(wxMDIParentFrame)))
    {
        ...
        AddPane(client_window,
                ewxAuiPaneInfo().Name(wxT("mdiclient")).
                CenterPane().PaneBorder(false));
    }
     else if (m_frame->IsKindOf(CLASSINFO(ewxAuiMDIParentFrame)))
    {
        ewxAuiMDIParentFrame* mdi_frame = (ewxAuiMDIParentFrame*)m_frame;
        ewxAuiMDIClientWindow* client_window = mdi_frame->GetClientWindow();
        wxASSERT_MSG(client_window, wxT("Client window is NULL!"));
...
wxWindow* ewxAuiManager::GetManagedWindow() const
{
    return m_frame;
}

ewxAuiDockArt* ewxAuiManager::GetArtProvider() const
{
    return m_art;
}

void ewxAuiManager::ProcessMgrEvent(ewxAuiManagerEvent& event)
{
...
// SetArtProvider() instructs ewxAuiManager to use the
// specified art provider for all drawing calls.  This allows
// plugable look-and-feel features.  The pointer that is
// passed to this method subsequently belongs to ewxAuiManager,
...
bool ewxAuiManager::AddPane(wxWindow* window, const ewxAuiPaneInfo& pane_info)
{
    ...
    ewxAuiPaneInfo& pinfo = m_panes.Last();
    ...
    {
        ewxAuiPaneButton button;
        button.button_id = ewxAUI_BUTTON_MAXIMIZE_RESTORE;
        pinfo.buttons.Add(button);
    }
    ...
    {
        ewxAuiPaneButton button;
        button.button_id = ewxAUI_BUTTON_PIN;
        pinfo.buttons.Add(button);
    }
    ...
    {
        ewxAuiPaneButton button;
        button.button_id = ewxAUI_BUTTON_CLOSE;
        pinfo.buttons.Add(button);
    }

    if (pinfo.HasGripper())
    {
        if (pinfo.window->IsKindOf(CLASSINFO(ewxAuiToolBar)))
        {
            // prevent duplicate gripper -- both ewxAuiManager and ewxAuiToolBar
            // have a gripper control.  The toolbar's built-in gripper
            // meshes better with the look and feel of the control than ours,
            // so turn ewxAuiManager's gripper off, and the toolbar's on.

            ewxAuiToolBar* tb = wx_static_cast(ewxAuiToolBar*, pinfo.window);
            pinfo.SetFlag(ewxAuiPaneInfo::optionGripper, false);
...
bool ewxAuiManager::AddPane(wxWindow* window,
                           int direction,
                           ...
                           const wxPoint& drop_pos)
{
    ...
    ewxAuiPaneInfo& pane = GetPane(window);
...
bool ewxAuiManager::InsertPane(wxWindow* window, const ewxAuiPaneInfo& pane_info,
                                int insert_level)
{
    wxASSERT_MSG(window, wxT("NULL window ptrs are not allowed"));
    ...
    switch (insert_level)
    {
        case ewxAUI_INSERT_PANE:
            DoInsertPane(m_panes,
                 ...
                 pane_info.dock_pos);
            break;
        case ewxAUI_INSERT_ROW:
            DoInsertDockRow(m_panes,
                 pane_info.dock_direction,
                 pane_info.dock_layer,
                 pane_info.dock_row);
            break;
        case ewxAUI_INSERT_DOCK:
    ...
    ewxAuiPaneInfo& existing_pane = GetPane(window);
...
bool ewxAuiManager::DetachPane(wxWindow* window)
{
    wxASSERT_MSG(window, wxT("NULL window ptrs are not allowed"));
    ...
    for (i = 0, count = m_panes.GetCount(); i < count; ++i)
    {
        ...
        if (p.window == window)
        {
            ...
            {
                ewxAuiDockUIPart& part = m_uiparts.Item(pi);
                if (part.pane == &p)
                {
...
void ewxAuiManager::ClosePane(ewxAuiPaneInfo& pane_info)
{
...
void ewxAuiManager::MaximizePane(ewxAuiPaneInfo& pane_info)
{
    int i, pane_count;
    ...
    for (i = 0, pane_count = m_panes.GetCount(); i < pane_count; ++i)
    {
        ...
        if (!p.IsToolbar() && !p.IsFloating())
        {
            ...
            // save hidden state
            p.SetFlag(ewxAuiPaneInfo::savedHiddenState,
                      p.HasFlag(ewxAuiPaneInfo::optionHidden));
...
void ewxAuiManager::RestorePane(ewxAuiPaneInfo& pane_info)
{
    int i, pane_count;
    ...
    for (i = 0, pane_count = m_panes.GetCount(); i < pane_count; ++i)
    {
        ...
        if (!p.IsToolbar() && !p.IsFloating())
        {
            p.SetFlag(ewxAuiPaneInfo::optionHidden,
                      p.HasFlag(ewxAuiPaneInfo::savedHiddenState));
        }
    }
...
void ewxAuiManager::RestoreMaximizedPane()
{
    int i, pane_count;
...
wxString ewxAuiManager::SavePaneInfo(ewxAuiPaneInfo& pane)
{
...
void ewxAuiManager::LoadPaneInfo(wxString pane_part, ewxAuiPaneInfo &pane)
{
...
wxString ewxAuiManager::SavePerspective()
{
    wxString result;
    result.Alloc(500);
    result = wxT("layout2|");
    ...
    {
        ewxAuiPaneInfo& pane = m_panes.Item(pane_i);
    ...
    {
        ewxAuiDockInfo& dock = m_docks.Item(dock_i);
...
bool ewxAuiManager::LoadPerspective(const wxString& layout, bool update)
{
    wxString input = layout;
    wxString part;

    // check layout string version
    //    'layout1' = ewxAUI 0.9.0 - ewxAUI 0.9.2
    //    'layout2' = ewxAUI 0.9.2 (wxWidgets 2.8)
    ...
    while (1)
    {
        ...
        ewxAuiPaneInfo& p = GetPane(pane.name);
        if (!p.IsOk())
        {
...
void ewxAuiManager::GetPanePositionsAndSizes(ewxAuiDockInfo& dock,
                                            wxArrayInt& positions,
                                            wxArrayInt& sizes)
{
    int caption_size = m_art->GetMetric(ewxAUI_DOCKART_CAPTION_SIZE);
    int pane_border_size = m_art->GetMetric(ewxAUI_DOCKART_PANE_BORDER_SIZE);
    int gripper_size = m_art->GetMetric(ewxAUI_DOCKART_GRIPPER_SIZE);
    ...
    for (pane_i = 0; pane_i < pane_count; ++pane_i)
    {
        ewxAuiPaneInfo& pane = *(dock.panes.Item(pane_i));

        if (pane.state & ewxAuiPaneInfo::actionPane)
        {
...
void ewxAuiManager::LayoutAddPane(wxSizer* cont,
                                 ewxAuiDockInfo& dock,
                                 ewxAuiPaneInfo& pane,
                                 ewxAuiDockUIPartArray& uiparts,
                                 bool spacer_only)
{
    ewxAuiDockUIPart part;
    wxSizerItem* sizer_item;

    int caption_size = m_art->GetMetric(ewxAUI_DOCKART_CAPTION_SIZE);
    int gripper_size = m_art->GetMetric(ewxAUI_DOCKART_GRIPPER_SIZE);
    int pane_border_size = m_art->GetMetric(ewxAUI_DOCKART_PANE_BORDER_SIZE);
    int pane_button_size = m_art->GetMetric(ewxAUI_DOCKART_PANE_BUTTON_SIZE);
    ...
    if (pane.HasGripper())
    {
        ...
        part.type = ewxAuiDockUIPart::typeGripper;
        part.dock = &dock;
        part.pane = &pane;
        part.button = NULL;
    ...
    if (pane.HasCaption())
    {
        ...
        part.type = ewxAuiDockUIPart::typeCaption;
        part.dock = &dock;
        part.pane = &pane;
        part.button = NULL;
        part.orientation = orientation;
    ...
    part.type = ewxAuiDockUIPart::typePane;
    part.dock = &dock;
    part.pane = &pane;
    part.button = NULL;
    part.orientation = orientation;
    ...
    if (pane.HasBorder())
    {
        ...
        part.type = ewxAuiDockUIPart::typePaneBorder;
        part.dock = &dock;
        part.pane = &pane;
        part.button = NULL;
        part.orientation = orientation;
...
void ewxAuiManager::LayoutAddDock(wxSizer* cont,
                                 ewxAuiDockInfo& dock,
                                 ewxAuiDockUIPartArray& uiparts,
                                 bool spacer_only)
{
    ...
    int sash_size = m_art->GetMetric(ewxAUI_DOCKART_SASH_SIZE);
    int orientation = dock.IsHorizontal() ? wxHORIZONTAL : wxVERTICAL;

    // resizable bottom and right docks have a sash before them
    if (!m_has_maximized && !dock.fixed && (dock.dock_direction == ewxAUI_DOCK_BOTTOM ||
                        dock.dock_direction == ewxAUI_DOCK_RIGHT))
    {
        sizer_item = cont->Add(sash_size, sash_size, 0, wxEXPAND);

        part.type = ewxAuiDockUIPart::typeDockSizer;
        part.orientation = orientation;
        part.dock = &dock;
        part.pane = NULL;
        part.button = NULL;
    ...
    if (dock.fixed)
    {
        ...
        for (pane_i = 0; pane_i < pane_count; ++pane_i)
        {
            ...
            if (amount > 0)
            {
                ...
                part.type = ewxAuiDockUIPart::typeBackground;
                part.dock = &dock;
                part.pane = NULL;
                part.button = NULL;
    ...
    else
    {
        for (pane_i = 0; pane_i < pane_count; ++pane_i)
        {
            ...
            if (!m_has_maximized && pane_i > 0)
            {
                ...
                part.type = ewxAuiDockUIPart::typePaneSizer;
                part.dock = &dock;
    ...
    if (dock.dock_direction == ewxAUI_DOCK_CENTER || has_maximized_pane)
        sizer_item = cont->Add(dock_sizer, 1, wxEXPAND);
         else
        sizer_item = cont->Add(dock_sizer, 0, wxEXPAND);

    part.type = ewxAuiDockUIPart::typeDock;
    part.dock = &dock;
    part.pane = NULL;
    part.button = NULL;
    part.orientation = orientation;
    ...
    if (!m_has_maximized &&
        !dock.fixed &&
          (dock.dock_direction == ewxAUI_DOCK_TOP ||
           dock.dock_direction == ewxAUI_DOCK_LEFT))
    {
        sizer_item = cont->Add(sash_size, sash_size, 0, wxEXPAND);
...
wxSizer* ewxAuiManager::LayoutAll(ewxAuiPaneInfoArray& panes,
                                 ...
                                 bool spacer_only)
{
    ...
    for (i = 0, dock_count = docks.GetCount(); i < dock_count; ++i)
    {
        ...
        if (dock.size == 0)
        {
            ...
            {
                ewxAuiPaneInfo& pane = *dock.panes.Item(j);
        ...
        for (j = 0; j < dock_pane_count; ++j)
        {
            ...
            if (!pane.IsToolbar())
                dock.toolbar = false;
            if (pane.HasFlag(ewxAuiPaneInfo::optionDockFixed))
                dock.fixed = true;
            if (pane.state & ewxAuiPaneInfo::actionPane)
    ...
    for (layer = 0; layer <= max_layer; ++layer)
    {
        ...
        FindDocks(docks, ewxAUI_DOCK_TOP, layer, -1, arr);
        if (!arr.IsEmpty())
        {
        ...
        FindDocks(docks, ewxAUI_DOCK_LEFT, layer, -1, arr);
        if (!arr.IsEmpty())
        {
            for (row = 0, row_count = arr.GetCount(); row < row_count; ++row)
        ...
        if (!old_cont)
        {
            ...
            FindDocks(docks, ewxAUI_DOCK_CENTER, -1, -1, arr);
            if (!arr.IsEmpty())
            {
                for (row = 0,row_count = arr.GetCount(); row<row_count; ++row)
                   LayoutAddDock(middle, *arr.Item(row), uiparts, spacer_only);
        ...
        FindDocks(docks, ewxAUI_DOCK_RIGHT, layer, -1, arr);
        if (!arr.IsEmpty())
        {
        ...
        FindDocks(docks, ewxAUI_DOCK_BOTTOM, layer, -1, arr);
        if (!arr.IsEmpty())
        {
            for (row = arr.GetCount()-1; row >= 0; --row)
                LayoutAddDock(cont, *arr.Item(row), uiparts, spacer_only);
...
void ewxAuiManager::SetDockSizeConstraint(double width_pct, double height_pct)
{
    m_dock_constraint_x = wxMax(0.0, wxMin(1.0, width_pct));
    m_dock_constraint_y = wxMax(0.0, wxMin(1.0, height_pct));
}

void ewxAuiManager::GetDockSizeConstraint(double* width_pct, double* height_pct) const
{
    if (width_pct)
...
void ewxAuiManager::Update()
{
    ...
    for (i = 0; i < pane_count; ++i)
    {
        ...
        if (p.IsFloating())
        {
            if (p.frame == NULL)
            {
                ...
                ewxAuiFloatingFrame* frame = CreateFloatingFrame(m_frame, p);

                // on MSW and Mac, if the owner desires transparent dragging, and
                // the dragging is happening right now, then the floating
                // window should have this style by default
                if (m_action == actionDragFloatingPane &&
                    (m_flags & ewxAUI_MGR_TRANSPARENT_DRAG))
             ...
             else
            {
                ...
                // and size reflect the information in ewxAuiPaneInfo
        ...
        if ((m_flags & ewxAUI_MGR_ALLOW_ACTIVE_PANE) == 0)
        {
            p.state &= ~ewxAuiPaneInfo::optionActive;
        }
    }
    ...
    ewxAuiRectArray old_pane_rects;
    for (i = 0; i < pane_count; ++i)
    {
        wxRect r;
        ewxAuiPaneInfo& p = m_panes.Item(i);
...
void ewxAuiManager::DoFrameLayout()
{
    m_frame->Layout();
    ...
    for (i = 0, part_count = m_uiparts.GetCount(); i < part_count; ++i)
    {
        ewxAuiDockUIPart& part = m_uiparts.Item(i);
        ...
        if (part.type == ewxAuiDockUIPart::typeDock)
            part.dock->rect = part.rect;
        if (part.type == ewxAuiDockUIPart::typePane)
...
ewxAuiDockUIPart* ewxAuiManager::GetPanePart(wxWindow* wnd)
{
    int i, part_count;
    for (i = 0, part_count = m_uiparts.GetCount(); i < part_count; ++i)
    {
        ewxAuiDockUIPart& part = m_uiparts.Item(i);
        if (part.type == ewxAuiDockUIPart::typePaneBorder &&
...
int ewxAuiManager::GetDockPixelOffset(ewxAuiPaneInfo& test)
{
    ...
    {
        ewxAuiDockUIPart& part = uiparts.Item(i);
        part.rect = wxRect(part.sizer_item->GetPosition(),
                           part.sizer_item->GetSize());
        if (part.type == ewxAuiDockUIPart::typeDock)
            part.dock->rect = part.rect;
...
bool ewxAuiManager::ProcessDockResult(ewxAuiPaneInfo& target,
                                     const ewxAuiPaneInfo& new_pos)
{
    bool allowed = false;
    switch (new_pos.dock_direction)
    {
        case ewxAUI_DOCK_TOP:    allowed = target.IsTopDockable();    break;
        case ewxAUI_DOCK_BOTTOM: allowed = target.IsBottomDockable(); break;
        case ewxAUI_DOCK_LEFT:   allowed = target.IsLeftDockable();   break;
        case ewxAUI_DOCK_RIGHT:  allowed = target.IsRightDockable();  break;
    }
...
// If, however, ewxAuiManager:m_docks and ewxAuiManager::m_panes are specified
...
bool ewxAuiManager::DoDrop(ewxAuiDockInfoArray& docks,
                          ewxAuiPaneInfoArray& panes,
                          ewxAuiPaneInfo& target,
                          const wxPoint& pt,
                          const wxPoint& offset)
{
    wxSize cli_size = m_frame->GetClientSize();

    ewxAuiPaneInfo drop = target;
    ...
    {
        int new_layer = wxMax(wxMax(GetMaxLayer(docks, ewxAUI_DOCK_LEFT),
                                GetMaxLayer(docks, ewxAUI_DOCK_BOTTOM)),
                                GetMaxLayer(docks, ewxAUI_DOCK_TOP)) + 1;
    ...
    {
        int new_layer = wxMax(wxMax(GetMaxLayer(docks, ewxAUI_DOCK_TOP),
                                GetMaxLayer(docks, ewxAUI_DOCK_LEFT)),
                                GetMaxLayer(docks, ewxAUI_DOCK_RIGHT)) + 1;
    ...
    {
        int new_layer = wxMax(wxMax(GetMaxLayer(docks, ewxAUI_DOCK_RIGHT),
                                GetMaxLayer(docks, ewxAUI_DOCK_TOP)),
                                GetMaxLayer(docks, ewxAUI_DOCK_BOTTOM)) + 1;
    ...
    {
        int new_layer = wxMax( wxMax( GetMaxLayer(docks, ewxAUI_DOCK_BOTTOM),
                                      GetMaxLayer(docks, ewxAUI_DOCK_LEFT)),
                                      GetMaxLayer(docks, ewxAUI_DOCK_RIGHT)) + 1;
    ...
    ewxAuiDockUIPart* part = HitTest(pt.x, pt.y);
    ...
    if (drop.IsToolbar())
    {
        ...
        if (!part->dock->fixed || part->dock->dock_direction == ewxAUI_DOCK_CENTER ||
            pt.x >= cli_size.x || pt.x <= 0 || pt.y >= cli_size.y || pt.y <= 0)
        {
            ...
            {
                if ((m_flags & ewxAUI_MGR_ALLOW_FLOATING) && drop.IsFloatable())
                {
                    drop.Float();
                }
        ...
        {
            if ((part->dock->dock_direction == ewxAUI_DOCK_TOP) ||
                (part->dock->dock_direction == ewxAUI_DOCK_LEFT))
            {
    ...
    if (part->type == ewxAuiDockUIPart::typePaneBorder ||
        ...
        part->type == ewxAuiDockUIPart::typeBackground)
    {
        if (part->type == ewxAuiDockUIPart::typeDockSizer)
        {
        ...
        if (part->dock && part->dock->toolbar)
        {
            ...
            {
                case ewxAUI_DOCK_LEFT:
                    layer = wxMax(wxMax(GetMaxLayer(docks, ewxAUI_DOCK_LEFT),
                                      GetMaxLayer(docks, ewxAUI_DOCK_BOTTOM)),
                                      GetMaxLayer(docks, ewxAUI_DOCK_TOP));
                    break;
                case ewxAUI_DOCK_TOP:
                    layer = wxMax(wxMax(GetMaxLayer(docks, ewxAUI_DOCK_TOP),
                                      GetMaxLayer(docks, ewxAUI_DOCK_LEFT)),
                                      GetMaxLayer(docks, ewxAUI_DOCK_RIGHT));
                    break;
                case ewxAUI_DOCK_RIGHT:
                    layer = wxMax(wxMax(GetMaxLayer(docks, ewxAUI_DOCK_RIGHT),
                                      GetMaxLayer(docks, ewxAUI_DOCK_TOP)),
                                      GetMaxLayer(docks, ewxAUI_DOCK_BOTTOM));
                    break;
                case ewxAUI_DOCK_BOTTOM:
                    layer = wxMax(wxMax(GetMaxLayer(docks, ewxAUI_DOCK_BOTTOM),
                                      GetMaxLayer(docks, ewxAUI_DOCK_LEFT)),
                                      GetMaxLayer(docks, ewxAUI_DOCK_RIGHT));
                    break;
            }
...
void ewxAuiManager::OnHintFadeTimer(wxTimerEvent& WXUNUSED(event))
{
...
void ewxAuiManager::ShowHint(const wxRect& rect)
{
    if (m_hint_wnd)
    {
        ...
        if ((m_flags & ewxAUI_MGR_HINT_FADE)
            && !((m_hint_wnd->IsKindOf(CLASSINFO(ewxPseudoTransparentFrame))) &&
                 (m_flags & ewxAUI_MGR_NO_VENETIAN_BLINDS_FADE))
            )
            m_hint_fadeamt = 0;
    ...
    {
        if (!(m_flags & ewxAUI_MGR_RECTANGLE_HINT))
            return;
        ...
        for (i = 0, pane_count = m_panes.GetCount(); i < pane_count; ++i)
        {
            ewxAuiPaneInfo& pane = m_panes.Item(i);
...
void ewxAuiManager::StartPaneDrag(wxWindow* pane_window,
                                 const wxPoint& offset)
{
    ewxAuiPaneInfo& pane = GetPane(pane_window);
    if (!pane.IsOk())
        return;
...
wxRect ewxAuiManager::CalculateHintRect(wxWindow* pane_window,
                                       const wxPoint& pt,
                                       const wxPoint& offset)
{
    wxRect rect;
    ...
    ewxAuiPaneInfoArray panes;
    ewxAuiDockUIPartArray uiparts;
    ewxAuiPaneInfo hint = GetPane(pane_window);
...
void ewxAuiManager::DrawHintRect(wxWindow* pane_window,
                                const wxPoint& pt,
                                const wxPoint& offset)
{
...
void ewxAuiManager::OnFloatingPaneMoveStart(wxWindow* wnd)
{
    // try to find the pane
    ewxAuiPaneInfo& pane = GetPane(wnd);
    wxASSERT_MSG(pane.IsOk(), wxT("Pane window not found"));

    if(!pane.frame)
        return;

    if (m_flags & ewxAUI_MGR_TRANSPARENT_DRAG)
        pane.frame->SetTransparent(150);
}

void ewxAuiManager::OnFloatingPaneMoving(wxWindow* wnd, wxDirection dir)
{
    // try to find the pane
    ewxAuiPaneInfo& pane = GetPane(wnd);
    wxASSERT_MSG(pane.IsOk(), wxT("Pane window not found"));
...
void ewxAuiManager::OnFloatingPaneMoved(wxWindow* wnd, wxDirection dir)
{
    // try to find the pane
    ewxAuiPaneInfo& pane = GetPane(wnd);
    wxASSERT_MSG(pane.IsOk(), wxT("Pane window not found"));
...
void ewxAuiManager::OnFloatingPaneResized(wxWindow* wnd, const wxSize& size)
{
    // try to find the pane
    ewxAuiPaneInfo& pane = GetPane(wnd);
    wxASSERT_MSG(pane.IsOk(), wxT("Pane window not found"));
...
void ewxAuiManager::OnFloatingPaneClosed(wxWindow* wnd, wxCloseEvent& evt)
{
    // try to find the pane
    ewxAuiPaneInfo& pane = GetPane(wnd);
    wxASSERT_MSG(pane.IsOk(), wxT("Pane window not found"));
    ...
    ewxAuiManagerEvent e(wxEVT_EWXAUI_PANE_CLOSE);
    e.SetPane(&pane);
     ...
     else
    {
        ...
        ewxAuiPaneInfo& check = GetPane(wnd);
        if (check.IsOk())
        {
            ClosePane(pane);
        }
...
void ewxAuiManager::OnFloatingPaneActivated(wxWindow* wnd)
{
    if ((GetFlags() & ewxAUI_MGR_ALLOW_ACTIVE_PANE) && GetPane(wnd).IsOk())
    {
...
void ewxAuiManager::OnRender(ewxAuiManagerEvent& evt)
{
...
// ewxAuiManager::OnRender().  This allows the render function to
...
void ewxAuiManager::Render(wxDC* dc)
{
    ewxAuiManagerEvent e(wxEVT_EWXAUI_RENDER);
    e.SetManager(this);
    e.SetDC(dc);
    ProcessMgrEvent(e);
}

void ewxAuiManager::Repaint(wxDC* dc)
{
#ifdef __WXMAC__
    if ( dc == NULL )
    {
...
void ewxAuiManager::OnPaint(wxPaintEvent& WXUNUSED(event))
{
    wxPaintDC dc(m_frame);
    Repaint(&dc);
}

void ewxAuiManager::OnEraseBackground(wxEraseEvent& event)
{
#ifdef __WXMAC__
    event.Skip() ;
#else
...
void ewxAuiManager::OnSize(wxSizeEvent& event)
{
    if (m_frame)
    {
        DoFrameLayout();
...
void ewxAuiManager::OnFindManager(ewxAuiManagerEvent& evt)
{
    ...
    if (window->IsKindOf(CLASSINFO(ewxAuiFloatingFrame)))
    {
        ewxAuiFloatingFrame* float_frame = wx_static_cast(ewxAuiFloatingFrame*, window);
...
void ewxAuiManager::OnSetCursor(wxSetCursorEvent& event)
{
    // determine cursor
    ewxAuiDockUIPart* part = HitTest(event.GetX(), event.GetY());
    ...
    if (part)
    {
            ...
            part->type == ewxAuiDockUIPart::typePaneSizer)
        {
            ...
            if (part->type == ewxAuiDockUIPart::typeDockSizer && part->dock &&
                part->dock->panes.GetCount() == 1 &&
         ...
         else if (part->type == ewxAuiDockUIPart::typeGripper)
        {
...
void ewxAuiManager::UpdateButtonOnScreen(ewxAuiDockUIPart* button_ui_part,
                                        const wxMouseEvent& event)
{
    ewxAuiDockUIPart* hit_test = HitTest(event.GetX(), event.GetY());
    if (!hit_test || !button_ui_part)
        return;

    int state = ewxAUI_BUTTON_STATE_NORMAL;

    if (hit_test == button_ui_part)
    {
        if (event.LeftDown())
            state = ewxAUI_BUTTON_STATE_PRESSED;
             else
            state = ewxAUI_BUTTON_STATE_HOVER;
    }
     else
    {
        if (event.LeftDown())
...
void ewxAuiManager::OnLeftDown(wxMouseEvent& event)
{
    ...
    if (part)
    {
            ...
            part->type == ewxAuiDockUIPart::typePaneSizer)
        {
            ...
            //if (part->dock && part->dock->dock_direction == ewxAUI_DOCK_CENTER)
            //    return;
         ...
         else if (part->type == ewxAuiDockUIPart::typePaneButton)
        {
         ...
         else if (part->type == ewxAuiDockUIPart::typeCaption ||
                  part->type == ewxAuiDockUIPart::typeGripper)
        {
            // if we are managing a ewxAuiFloatingFrame window, then
            // we are an embedded ewxAuiManager inside the ewxAuiFloatingFrame.
            ...
            if (part->pane &&
                part->pane->window &&
                managed_wnd &&
                managed_wnd->IsKindOf(CLASSINFO(ewxAuiFloatingFrame)))
            {
                ewxAuiFloatingFrame* floating_frame = (ewxAuiFloatingFrame*)managed_wnd;
                ewxAuiManager* owner_mgr = floating_frame->GetOwnerManager();
...
*/
            ...
            if (GetFlags() & ewxAUI_MGR_ALLOW_ACTIVE_PANE)
            {
...
bool ewxAuiManager::DoEndResizeAction(wxMouseEvent& event)
{
    // resize the dock or the pane
    if (m_action_part && m_action_part->type==ewxAuiDockUIPart::typeDockSizer)
    {
        ...
        {
            ewxAuiDockInfo& dock = m_docks.Item(dock_i);
            if (dock.dock_direction == ewxAUI_DOCK_TOP ||
                dock.dock_direction == ewxAUI_DOCK_BOTTOM)
            {
                used_height += dock.size;
            }
            if (dock.dock_direction == ewxAUI_DOCK_LEFT ||
                dock.dock_direction == ewxAUI_DOCK_RIGHT)
            {
        ...
        m_action_part->type == ewxAuiDockUIPart::typePaneSizer)
    {
        ewxAuiDockInfo& dock = *m_action_part->dock;
        ewxAuiPaneInfo& pane = *m_action_part->pane;
        ...
        ewxAuiDockUIPart* pane_part = GetPanePart(pane.window);
...
void ewxAuiManager::OnLeftUp(wxMouseEvent& event)
{
    if (m_action == actionResize)
    {
        ...
        if (!ewxAuiManager_HasLiveResize(*this))
        {
            // get rid of the hint rectangle
#if defined(__WXMAC__) && !wxMAC_USE_CORE_GRAPHICS
        ...
        if (gs_CurrentDragItem != -1 && ewxAuiManager_HasLiveResize(*this))
    ...
    else if (m_action == actionClickButton)
    {
        ...
        if (m_action_part)
        {
            ...
            if (m_action_part == HitTest(event.GetX(), event.GetY()))
            {
                ...
                ewxAuiManagerEvent e(wxEVT_EWXAUI_PANE_BUTTON);
                e.SetManager(this);
    ...
    else if (m_action == actionDragToolbarPane)
    {
        ...
        ewxAuiPaneInfo& pane = GetPane(m_action_window);
        wxASSERT_MSG(pane.IsOk(), wxT("Pane window not found"));

        // save the new positions
        ewxAuiDockInfoPtrArray docks;
        FindDocks(m_docks, pane.dock_direction,
                  pane.dock_layer, pane.dock_row, docks);
        if (docks.GetCount() == 1)
        {
            ewxAuiDockInfo& dock = *docks.Item(0);
        ...
        pane.state &= ~ewxAuiPaneInfo::actionPane;
        Update();
    }
    else
    {
...
void ewxAuiManager::OnMotion(wxMouseEvent& event)
{
    ...
    else if (m_action == actionClickCaption)
    {
        ...
        {
            ewxAuiPaneInfo* pane_info = m_action_part->pane;

            if (!pane_info->IsToolbar())
            {
                if ((m_flags & ewxAUI_MGR_ALLOW_FLOATING) &&
    ...
    {
        ewxAuiDockUIPart* part = HitTest(event.GetX(), event.GetY());
        if (part && part->type == ewxAuiDockUIPart::typePaneButton)
        {
...
void ewxAuiManager::OnLeaveWindow(wxMouseEvent& WXUNUSED(event))
{
    if (m_hover_button)
    {
        m_hover_button = NULL;
...
void ewxAuiManager::OnChildFocus(wxChildFocusEvent& event)
{
    ...
    if (GetFlags() & ewxAUI_MGR_ALLOW_ACTIVE_PANE)
    {
        ewxAuiPaneInfo& pane = GetPane(event.GetWindow());
        if (pane.IsOk() && (pane.state & ewxAuiPaneInfo::optionActive) == 0)
        {
...
void ewxAuiManager::OnPaneButton(ewxAuiManagerEvent& evt)
{
    wxASSERT_MSG(evt.pane, wxT("Pane Info passed to ewxAuiManager::OnPaneButton must be non-null"));

    ewxAuiPaneInfo& pane = *(evt.pane);

    if (evt.button == ewxAUI_BUTTON_CLOSE)
    {
        // fire pane close event
        ewxAuiManagerEvent e(wxEVT_EWXAUI_PANE_CLOSE);
        e.SetManager(this);
        ...
        if (!e.GetVeto())
        {
            ...
            ewxAuiPaneInfo& check = GetPane(pane.window);
            if (check.IsOk())
            {
                ClosePane(pane);
            }
    ...
    else if (evt.button == ewxAUI_BUTTON_MAXIMIZE_RESTORE && !pane.IsMaximized())
    {
        // fire pane close event
        ewxAuiManagerEvent e(wxEVT_EWXAUI_PANE_MAXIMIZE);
        e.SetManager(this);
        e.SetPane(evt.pane);
        ProcessMgrEvent(e);
    ...
    {
        // fire pane close event
        ewxAuiManagerEvent e(wxEVT_EWXAUI_PANE_RESTORE);
        e.SetManager(this);
        e.SetPane(evt.pane);
        ProcessMgrEvent(e);
    ...
    else if (evt.button == ewxAUI_BUTTON_OPTIONS)
    {
     ...
     else if (evt.button == ewxAUI_BUTTON_PIN)
