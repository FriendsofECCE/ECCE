...
void CalcEd::OnButtonCalcedVerifyClick( wxCommandEvent& event )
{
  if (p_iCalc) {
    try {
      //  Check what would actually be submitted, which means saving
      //  first if the editor has unsaved changes -- the input file is
      //  regenerated by the save.  Checking a stale file and reporting
      //  on it as though it were current would be worse than not
      //  checking at all.
      if (p_feedback->getEditStatus() == WxFeedback::MODIFIED)
        doSave();
