...
double wxPlotData::Get2Norm(int start_index, int end_index) const
{
    wxCHECK_MSG( Ok(), 0.0, wxT("Invalid wxPlotData") );
    const int count = M_PLOTDATA->m_count - start_index;
    if (end_index < 0) end_index = count - 1;
    CHECK_START_END_INDEX_MSG(start_index, end_index, count, 0);
    ...
    if (M_PLOTDATA->m_Yidata)
    {
        ...
        {
            norm2 += y_data[i]*y_data[i] + yi_data[i]*yi_data[i];
        }
    }
    else
    {
        for (int i = start_index; i <= end_index; i++)
        {
            norm2 += y_data[i]*y_data[i];
        }
    }

    double range = GetXValue(end_index)-GetXValue(start_index);
    norm2 = sqrt( norm2/(double)count * range);
...
{
    PowerXY(power, 1, start_index, end_index);
}
void wxPlotData::PowerY( double power, int start_index, int end_index )
{
    PowerXY(1, power, start_index, end_index);
}
...
//----------------------------------------------------------------------------
wxPlotData wxPlotData::FFT( bool forward ) const
{
    wxCHECK_MSG( Ok(), wxPlotData(), wxT("Invalid wxPlotData") );
    ...
    if (forward)
    {
        ...
        trans_xData[0] = 0.0;
        double timestep = (M_PLOTDATA->m_Xdata[1] - M_PLOTDATA->m_Xdata[0])*(samples-1);
...
*/
    ...
    {
        double *trans_xData  = trans.GetXData();
        double timestep = 1.0/((M_PLOTDATA->m_Xdata[1] - M_PLOTDATA->m_Xdata[0])*double(samples-1));
        for (int i = 0; i < samples; i++)
        {
...
wxPlotData wxPlotData::PowerSpectrum() const
{
    ...
    double *y_data  = power.GetYData();
    double *yi_data = power.GetYiData();
    double N2 = GetCount()*GetCount(); // normalization factor

    for (i = 0; i < count; i++)
    {
        y_data[i] = (y_data[i]*y_data[i] + yi_data[i]*yi_data[i])/N2;
    }

    power.SetYiData(NULL);
...
wxPlotData wxPlotData::OneSidedPowerSpectrum() const
{
    wxCHECK_MSG( Ok(), wxPlotData(), wxT("Invalid wxPlotData, wxPlotData::RunAverage()") );
    ...
    y_data[0] = 2.0*(y_data[0]*y_data[0] + yi_data[0]*yi_data[0]);

    for (i = 1; i < count/2; i++)
    {
        y_data[i] = y_data[i]*y_data[i] + yi_data[i]*yi_data[i] +
                    y_data[count-i]*y_data[count-i] + yi_data[count-i]*yi_data[count-i];
    }
    y_data[count/2] = 2.0*(y_data[count/2]*y_data[count/2] +
                           yi_data[count/2]*yi_data[count/2]);

    power.SetYiData(NULL);

    power.Resize(count/2+1);

    double pnorm = power.Get2Norm(0, -1);

    power.ScaleY((norm*norm)/(pnorm*pnorm), 0.0, 0, -1);
...
wxPlotData wxPlotData::FFTHiPassFilter( double fc, wxPlotData::FFTFilter_Type filter, double n ) const
{
...
wxPlotData wxPlotData::FFTLoPassFilter( double fc, wxPlotData::FFTFilter_Type filter, double n ) const
{
    wxCHECK_MSG( Ok() && (n>0), wxPlotData(), wxT("Invalid wxPlotData") );
...
wxPlotData wxPlotData::FFTNotchFilter( double lo, double hi, wxPlotData::FFTFilter_Type filter, double n ) const
{
...
wxPlotData wxPlotData::FFTBandPassFilter( double lo, double hi, wxPlotData::FFTFilter_Type filter, double n ) const
{
    wxPlotData xform;
...
wxPlotData wxPlotData::FFTCustomFilter( const wxPlotFunction &func ) const
{
