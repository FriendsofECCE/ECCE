...
extern "C" {
    ...
    #define INITIALIZE_FAST_GRAPHICS \
        ...
        wxWindowDCImpl *winDCImpl = wxDynamicCast(dc->GetImpl(), wxWindowDCImpl); \
        GdkWindow *window = NULL; \
        GdkGC     *pen = NULL; \
        if (winDCImpl && (dc_scale_x == 1.0) && (dc_scale_y == 1.0) && (dc_origin == wxPoint(0,0))) \
        { \
            window = winDCImpl->m_gdkwindow; \
            pen = winDCImpl->m_penGC; \
        }
#else
    #define INITIALIZE_FAST_GRAPHICS \
        double dc_scale_x = 1, dc_scale_y = 1; \
...
*/
        ...
        else // figure out the rest
        {
            ...
            if ((marker_type == wxPLOTMARKER_POINT)) // || (((x0 == x1) && (y0 == y1)) && (marker_type != wxPLOTMARKER_RECT)))
            {
                // only draw for points since it doesn't honor the pen size style
            ...
            {
                if ((r.m_y >= subViewRect.m_y) && (r.m_y <= subViewRect.GetBottom()))
                {
                    double y = r.m_y;
                ...
                if ((r.m_x >= subViewRect.m_x) && (r.m_x <= subViewRect.GetRight()))
                {
                    double x = r.m_x;
                    ClipLineToRect(x, y0, x, y1, subViewRect);
                    int i0 = m_owner->GetClientCoordFromPlotX(x);
                    int j0 = m_owner->GetClientCoordFromPlotY(y0);
                    int j1 = m_owner->GetClientCoordFromPlotY(y1);
                    wxPLOTCTRL_DRAW_LINE(dc, window, pen, i0, j0, i0, j1);
                }
            ...
            else // rectangle
            {
                ...
                bool is_using_pixel_size = (marker_type == wxPLOTMARKER_RECT) && (size.GetWidth() > 0) && (size.GetHeight() > 0);

                if (is_using_pixel_size)
                {
                    // figure out size on plot for clipping
                    wxRect2DDouble r(m_owner->GetPlotRectFromClientRect(wxRect(wxPoint(0,0), size)));
                    clippedRect.m_x     -= r.m_width/2;
                    clippedRect.m_width  = r.m_width;
                    clippedRect.m_y     -= r.m_height/2;
                    clippedRect.m_height = r.m_height;
                }

                clippedRect.Intersect(subViewRect);

                if ((clippedRect.m_width > 0) && (clippedRect.m_height > 0))
                {
                    int i0 = 0, i1 = 0, j0 = 0, j1 = 0;

                    if (is_using_pixel_size)
                    {
                        i0 = m_owner->GetClientCoordFromPlotX(x0);
                        j0 = m_owner->GetClientCoordFromPlotY(y0);

                        // Draw rect of pixel wxSize centered at the wxRect2DDouble's position
                        i0 -= (int)(size.GetWidth()/2.0);
                        i1  = i0 + size.GetWidth();
                        j0 -= (int)(size.GetHeight()/2.0);
                        j1  = j0 + size.GetHeight();
                    }
                    else
                    {
                        i0 = m_owner->GetClientCoordFromPlotX(clippedRect.m_x);
                        i1 = m_owner->GetClientCoordFromPlotX(clippedRect.GetRight());
                        j0 = m_owner->GetClientCoordFromPlotY(clippedRect.m_y);
                        j1 = m_owner->GetClientCoordFromPlotY(clippedRect.GetBottom());

                        // push the edge of the rect outside the window so we don't crash
                        int pen_width = 2*dc->GetPen().GetWidth() + 5;
