...
WX_DEFINE_OBJARRAY(ewxAuiToolBarItemArray)
...
IMPLEMENT_CLASS(ewxAuiToolBar, wxControl)
IMPLEMENT_DYNAMIC_CLASS(ewxAuiToolBarEvent, wxEvent)
...
unsigned char ewxAuiBlendColour(unsigned char fg, unsigned char bg, double alpha);
...
static wxBitmap MakeDisabledBitmap(wxBitmap& bmp)
{
    ...
    for (int y = height-1; y >= 0; --y)
    {
        for (int x = width-1; x >= 0; --x)
        {
            ...
            *r = ewxAuiBlendColour(*r, 255, 0.4);
            *g = ewxAuiBlendColour(*g, 255, 0.4);
            *b = ewxAuiBlendColour(*b, 255, 0.4);
        }
    }
...
static const unsigned char
    DISABLED_TEXT_GREY_HUE = ewxAuiBlendColour(0, 255, 0.4);
const wxColour DISABLED_TEXT_COLOR(DISABLED_TEXT_GREY_HUE,
                                   DISABLED_TEXT_GREY_HUE,
                                   DISABLED_TEXT_GREY_HUE);

ewxAuiDefaultToolBarArt::ewxAuiDefaultToolBarArt()
{
    m_base_colour = GetBaseColor();

    m_flags = 0;
    m_text_orientation = ewxAUI_TBTOOL_TEXT_BOTTOM;
    ...
    wxColor darker1_colour = ewxAuiStepColour(m_base_colour, 85);
    wxColor darker2_colour = ewxAuiStepColour(m_base_colour, 75);
    wxColor darker3_colour = ewxAuiStepColour(m_base_colour, 60);
    wxColor darker4_colour = ewxAuiStepColour(m_base_colour, 50);
    wxColor darker5_colour = ewxAuiStepColour(m_base_colour, 40);
    ...
    m_button_dropdown_bmp = ewxAuiBitmapFromBits(button_dropdown_bits, 5, 3,
                                                *wxBLACK);
    m_disabled_button_dropdown_bmp = ewxAuiBitmapFromBits(
                                                button_dropdown_bits, 5, 3,
                                                wxColor(128,128,128));
    m_overflow_bmp = ewxAuiBitmapFromBits(overflow_bits, 7, 6, *wxBLACK);
    m_disabled_overflow_bmp = ewxAuiBitmapFromBits(overflow_bits, 7, 6, wxColor(128,128,128));
...
ewxAuiToolBarArt* ewxAuiDefaultToolBarArt::Clone()
{
    return static_cast<ewxAuiToolBarArt*>(new ewxAuiDefaultToolBarArt);
}

void ewxAuiDefaultToolBarArt::SetFlags(unsigned int flags)
{
    m_flags = flags;
}

void ewxAuiDefaultToolBarArt::SetFont(const wxFont& font)
{
    m_font = font;
}

void ewxAuiDefaultToolBarArt::SetTextOrientation(int orientation)
{
    m_text_orientation = orientation;
}

void ewxAuiDefaultToolBarArt::DrawBackground(
                                    wxDC& dc,
                                    ...
                                    const wxRect& _rect)
{
    wxRect rect = _rect;
    rect.height++;
    wxColour start_colour = ewxAuiStepColour(m_base_colour, 150);
    wxColour end_colour = ewxAuiStepColour(m_base_colour, 90);
    dc.GradientFillLinear(rect, start_colour, end_colour, wxSOUTH);
}

void ewxAuiDefaultToolBarArt::DrawLabel(
                                    wxDC& dc,
                                    wxWindow* WXUNUSED(wnd),
                                    const ewxAuiToolBarItem& item,
                                    const wxRect& rect)
{
    dc.SetFont(m_font);
...
void ewxAuiDefaultToolBarArt::DrawButton(
                                    wxDC& dc,
                                    wxWindow* WXUNUSED(wnd),
                                    const ewxAuiToolBarItem& item,
                                    const wxRect& rect)
{
    ...
    if (m_text_orientation == ewxAUI_TBTOOL_TEXT_BOTTOM)
    {
        bmp_x = rect.x +
                (rect.width/2) -
    ...
    else if (m_text_orientation == ewxAUI_TBTOOL_TEXT_RIGHT)
    {
        bmp_x = rect.x + 3;
    ...
    if (!(item.GetState() & ewxAUI_BUTTON_STATE_DISABLED))
    {
        if (item.GetState() & ewxAUI_BUTTON_STATE_PRESSED)
        {
            dc.SetPen(wxPen(m_highlight_colour));
            dc.SetBrush(wxBrush(ewxAuiStepColour(m_highlight_colour, 150)));
            dc.DrawRectangle(rect);
        }
        else if ((item.GetState() & ewxAUI_BUTTON_STATE_HOVER) || item.IsSticky())
        {
            dc.SetPen(wxPen(m_highlight_colour));
            dc.SetBrush(wxBrush(ewxAuiStepColour(m_highlight_colour, 170)));

            // draw an even lighter background for checked item hovers (since
            // the hover background is the same color as the check background)
            if (item.GetState() & ewxAUI_BUTTON_STATE_CHECKED)
                dc.SetBrush(wxBrush(ewxAuiStepColour(m_highlight_colour, 180)));

            dc.DrawRectangle(rect);
        }
        else if (item.GetState() & ewxAUI_BUTTON_STATE_CHECKED)
        {
    ...
    if ( (m_flags & ewxAUI_TB_TEXT) && !item.GetLabel().empty() )
    {
        dc.DrawText(item.GetLabel(), text_x, text_y);
    }
}
...
void ewxAuiDefaultToolBarArt::DrawDropDownButton(
                                    wxDC& dc,
                                    wxWindow* WXUNUSED(wnd),
                                    const ewxAuiToolBarItem& item,
                                    const wxRect& rect)
{
    ...
    {
        dc.SetPen(wxPen(m_highlight_colour));
        dc.SetBrush(wxBrush(ewxAuiStepColour(m_highlight_colour, 140)));
        dc.DrawRectangle(button_rect);
        dc.DrawRectangle(dropdown_rect);
    }
    else if (item.GetState() & ewxAUI_BUTTON_STATE_HOVER ||
             item.IsSticky())
    {
        dc.SetPen(wxPen(m_highlight_colour));
        dc.SetBrush(wxBrush(ewxAuiStepColour(m_highlight_colour, 170)));
...
void ewxAuiDefaultToolBarArt::DrawControlLabel(
                                    wxDC& dc,
                                    wxWindow* WXUNUSED(wnd),
                                    const ewxAuiToolBarItem& item,
                                    const wxRect& rect)
...
wxSize ewxAuiDefaultToolBarArt::GetLabelSize(
                                        wxDC& dc,
                                        wxWindow* WXUNUSED(wnd),
                                        const ewxAuiToolBarItem& item)
{
...
wxSize ewxAuiDefaultToolBarArt::GetToolSize(
                                        wxDC& dc,
                                        wxWindow* WXUNUSED(wnd),
                                        const ewxAuiToolBarItem& item)
{
    if (!item.GetBitmap().IsOk() && !(m_flags & ewxAUI_TB_TEXT))
        return wxSize(16,16);
...
void ewxAuiDefaultToolBarArt::DrawSeparator(
                                    wxDC& dc,
                                    wxWindow* WXUNUSED(wnd),
                                    const wxRect& _rect)
{
    bool horizontal = true;
    if (m_flags & ewxAUI_TB_VERTICAL)
        horizontal = false;
    ...
    wxColour start_colour = ewxAuiStepColour(m_base_colour, 80);
    wxColour end_colour = ewxAuiStepColour(m_base_colour, 80);
    dc.GradientFillLinear(rect, start_colour, end_colour, horizontal ? wxSOUTH : wxEAST);
}

void ewxAuiDefaultToolBarArt::DrawGripper(wxDC& dc,
                                    wxWindow* WXUNUSED(wnd),
                                    const wxRect& rect)
{
    int i = 0;
...
void ewxAuiDefaultToolBarArt::DrawOverflowButton(wxDC& dc,
                                          wxWindow* wnd,
                                          const wxRect& rect,
                                          int state)
{
    if (state & ewxAUI_BUTTON_STATE_HOVER ||
        state & ewxAUI_BUTTON_STATE_PRESSED)
    {
        wxRect cli_rect = wnd->GetClientRect();
        wxColor light_gray_bg = ewxAuiStepColour(m_highlight_colour, 170);
...
int ewxAuiDefaultToolBarArt::GetElementSize(int element_id)
{
    switch (element_id)
    {
        case ewxAUI_TBART_SEPARATOR_SIZE: return m_separator_size;
        case ewxAUI_TBART_GRIPPER_SIZE:   return m_gripper_size;
        case ewxAUI_TBART_OVERFLOW_SIZE:  return m_overflow_size;
        default: return 0;
    }
}

void ewxAuiDefaultToolBarArt::SetElementSize(int element_id, int size)
{
    switch (element_id)
    {
        case ewxAUI_TBART_SEPARATOR_SIZE: m_separator_size = size; break;
        case ewxAUI_TBART_GRIPPER_SIZE:   m_gripper_size = size; break;
        case ewxAUI_TBART_OVERFLOW_SIZE:  m_overflow_size = size; break;
    }
}

int ewxAuiDefaultToolBarArt::ShowDropDown(wxWindow* wnd,
                                         const ewxAuiToolBarItemArray& items)
{
    wxMenu menuPopup;
    ...
    for (i = 0; i < count; ++i)
    {
        ewxAuiToolBarItem& item = items.Item(i);
...
BEGIN_EVENT_TABLE(ewxAuiToolBar, wxControl)
    EVT_SIZE(ewxAuiToolBar::OnSize)
    EVT_IDLE(ewxAuiToolBar::OnIdle)
    EVT_ERASE_BACKGROUND(ewxAuiToolBar::OnEraseBackground)
    EVT_PAINT(ewxAuiToolBar::OnPaint)
    EVT_LEFT_DOWN(ewxAuiToolBar::OnLeftDown)
    EVT_LEFT_DCLICK(ewxAuiToolBar::OnLeftDown)
    EVT_LEFT_UP(ewxAuiToolBar::OnLeftUp)
    EVT_RIGHT_DOWN(ewxAuiToolBar::OnRightDown)
    EVT_RIGHT_DCLICK(ewxAuiToolBar::OnRightDown)
    EVT_RIGHT_UP(ewxAuiToolBar::OnRightUp)
    EVT_MIDDLE_DOWN(ewxAuiToolBar::OnMiddleDown)
    EVT_MIDDLE_DCLICK(ewxAuiToolBar::OnMiddleDown)
    EVT_MIDDLE_UP(ewxAuiToolBar::OnMiddleUp)
    EVT_MOTION(ewxAuiToolBar::OnMotion)
    EVT_LEAVE_WINDOW(ewxAuiToolBar::OnLeaveWindow)
    EVT_SET_CURSOR(ewxAuiToolBar::OnSetCursor)
END_EVENT_TABLE()


ewxAuiToolBar::ewxAuiToolBar(wxWindow* parent,
                           wxWindowID id,
                                        ...
                                        style | wxBORDER_NONE)
{
    ...
    m_action_item = NULL;
    m_tip_item = NULL;
    m_art = new ewxAuiDefaultToolBarArt;
    m_tool_packing = 2;
    m_tool_border_padding = 3;
    m_tool_text_orientation = ewxAUI_TBTOOL_TEXT_BOTTOM;
    m_gripper_sizer_item = NULL;
    m_overflow_sizer_item = NULL;
    m_dragging = false;
    m_style = style | wxBORDER_NONE;
    m_gripper_visible = (m_style & ewxAUI_TB_GRIPPER) ? true : false;
    m_overflow_visible = (m_style & ewxAUI_TB_OVERFLOW) ? true : false;
    m_overflow_state = 0;
    SetMargins(5, 5, 2, 2);
    SetFont(*wxNORMAL_FONT);
    m_art->SetFlags((unsigned int)m_style);
    SetExtraStyle(wxWS_EX_PROCESS_IDLE);
    if (style & ewxAUI_TB_HORZ_LAYOUT)
        SetToolTextOrientation(ewxAUI_TBTOOL_TEXT_RIGHT);
}


ewxAuiToolBar::~ewxAuiToolBar()
{
    delete m_art;
    delete m_sizer;
}

void ewxAuiToolBar::SetWindowStyleFlag(long style)
{
    ...
    if (m_style & ewxAUI_TB_GRIPPER)
        m_gripper_visible = true;
    else
        m_gripper_visible = false;


    if (m_style & ewxAUI_TB_OVERFLOW)
    ...
    if (style & ewxAUI_TB_HORZ_LAYOUT)
        ...
        SetToolTextOrientation(ewxAUI_TBTOOL_TEXT_BOTTOM);
}

long ewxAuiToolBar::GetWindowStyleFlag() const
{
    return m_style;
}

void ewxAuiToolBar::SetArtProvider(ewxAuiToolBarArt* art)
{
    delete m_art;
...
ewxAuiToolBarArt* ewxAuiToolBar::GetArtProvider() const
{
    return m_art;
}
...
void ewxAuiToolBar::AddTool(int tool_id,
...
void ewxAuiToolBar::AddControl(wxControl* control,
                              const wxString& label)
{
    ewxAuiToolBarItem item;
...
void ewxAuiToolBar::AddLabel(int tool_id,
                            const wxString& label,
                            const int width)
{
...
void ewxAuiToolBar::AddSeparator()
{
    ewxAuiToolBarItem item;
    item.window = NULL;
...
void ewxAuiToolBar::AddSpacer(int pixels)
{
    ewxAuiToolBarItem item;
    item.window = NULL;
    item.label = wxEmptyString;
...
void ewxAuiToolBar::AddStretchSpacer(int proportion)
{
    ewxAuiToolBarItem item;
    item.window = NULL;
    item.label = wxEmptyString;
...
void ewxAuiToolBar::Clear()
{
    m_items.Clear();
    m_sizer_element_count = 0;
}

bool ewxAuiToolBar::DeleteTool(int tool_id)
{
...
bool ewxAuiToolBar::DeleteByIndex(int idx)
{
    if (idx >= 0 && idx < (int)m_items.GetCount())
    {
        m_items.RemoveAt(idx);
...
wxControl* ewxAuiToolBar::FindControl(int id)
{
    wxWindow* wnd = FindWindow(id);
    return (wxControl*)wnd;
}

ewxAuiToolBarItem* ewxAuiToolBar::FindTool(int tool_id) const
{
    size_t i, count;
    for (i = 0, count = m_items.GetCount(); i < count; ++i)
    {
        ewxAuiToolBarItem& item = m_items.Item(i);
        if (item.id == tool_id)
            return &item;
    }
...
ewxAuiToolBarItem* ewxAuiToolBar::FindToolByPosition(wxCoord x, wxCoord y) const
{
    size_t i, count;
    for (i = 0, count = m_items.GetCount(); i < count; ++i)
    {
...
ewxAuiToolBarItem* ewxAuiToolBar::FindToolByPositionWithPacking(wxCoord x, wxCoord y) const
{
    size_t i, count;
    for (i = 0, count = m_items.GetCount(); i < count; ++i)
    {
...
ewxAuiToolBarItem* ewxAuiToolBar::FindToolByIndex(int idx) const
{
    if (idx < 0)
        return NULL;
...
void ewxAuiToolBar::SetToolBitmapSize(const wxSize& WXUNUSED(size))
{
    // TODO: wxToolBar compatibility
}

wxSize ewxAuiToolBar::GetToolBitmapSize() const
{
    // TODO: wxToolBar compatibility
    return wxSize(16,15);
}

void ewxAuiToolBar::SetToolProportion(int tool_id, int proportion)
{
    ewxAuiToolBarItem* item = FindTool(tool_id);
    if (!item)
        return;
...
int ewxAuiToolBar::GetToolProportion(int tool_id) const
{
    ewxAuiToolBarItem* item = FindTool(tool_id);
    if (!item)
        return 0;
...
void ewxAuiToolBar::SetToolSeparation(int separation)
{
    if (m_art)
        m_art->SetElementSize(ewxAUI_TBART_SEPARATOR_SIZE, separation);
}

int ewxAuiToolBar::GetToolSeparation() const
{
    if (m_art)
        return m_art->GetElementSize(ewxAUI_TBART_SEPARATOR_SIZE);
    else
        return 5;
}


void ewxAuiToolBar::SetToolDropDown(int tool_id, bool dropdown)
{
    ewxAuiToolBarItem* item = FindTool(tool_id);
    if (!item)
        return;
...
bool ewxAuiToolBar::GetToolDropDown(int tool_id) const
{
    ewxAuiToolBarItem* item = FindTool(tool_id);
    if (!item)
        return 0;
...
void ewxAuiToolBar::SetToolSticky(int tool_id, bool sticky)
{
    // ignore separators
    if (tool_id == -1)
        return;
...
bool ewxAuiToolBar::GetToolSticky(int tool_id) const
{
    ewxAuiToolBarItem* item = FindTool(tool_id);
    if (!item)
        return 0;
...
void ewxAuiToolBar::SetToolBorderPadding(int padding)
{
    m_tool_border_padding = padding;
}

int ewxAuiToolBar::GetToolBorderPadding() const
{
    return m_tool_border_padding;
}

void ewxAuiToolBar::SetToolTextOrientation(int orientation)
{
...
int ewxAuiToolBar::GetToolTextOrientation() const
{
    return m_tool_text_orientation;
}

void ewxAuiToolBar::SetToolPacking(int packing)
{
    m_tool_packing = packing;
}

int ewxAuiToolBar::GetToolPacking() const
{
    return m_tool_packing;
}


void ewxAuiToolBar::SetOrientation(int WXUNUSED(orientation))
{
}

void ewxAuiToolBar::SetMargins(int left, int right, int top, int bottom)
{
    if (left != -1)
        m_left_padding = left;
    if (right != -1)
...
bool ewxAuiToolBar::GetGripperVisible() const
{
    return m_gripper_visible;
}

void ewxAuiToolBar::SetGripperVisible(bool visible)
{
    m_gripper_visible = visible;
    if (visible)
        m_style |= ewxAUI_TB_GRIPPER;
    else
        m_style &= ~ewxAUI_TB_GRIPPER;
    Realize();
    Refresh(false);
...
bool ewxAuiToolBar::GetOverflowVisible() const
{
    return m_overflow_visible;
}

void ewxAuiToolBar::SetOverflowVisible(bool visible)
{
    m_overflow_visible = visible;
    if (visible)
        m_style |= ewxAUI_TB_OVERFLOW;
    else
        m_style &= ~ewxAUI_TB_OVERFLOW;
    Refresh(false);
}

bool ewxAuiToolBar::SetFont(const wxFont& font)
{
...
void ewxAuiToolBar::SetHoverItem(ewxAuiToolBarItem* pitem)
{
    ewxAuiToolBarItem* former_hover = NULL;

    size_t i, count;
    for (i = 0, count = m_items.GetCount(); i < count; ++i)
    {
        ewxAuiToolBarItem& item = m_items.Item(i);
        if (item.state & ewxAUI_BUTTON_STATE_HOVER)
            former_hover = &item;
        item.state &= ~ewxAUI_BUTTON_STATE_HOVER;
    }

    if (pitem)
    {
        pitem->state |= ewxAUI_BUTTON_STATE_HOVER;
    }
...
void ewxAuiToolBar::SetPressedItem(ewxAuiToolBarItem* pitem)
{
    ewxAuiToolBarItem* former_item = NULL;

    size_t i, count;
    for (i = 0, count = m_items.GetCount(); i < count; ++i)
    {
        ewxAuiToolBarItem& item = m_items.Item(i);
        if (item.state & ewxAUI_BUTTON_STATE_PRESSED)
            former_item = &item;
        item.state &= ~ewxAUI_BUTTON_STATE_PRESSED;
    }

    if (pitem)
    {
        pitem->state &= ~ewxAUI_BUTTON_STATE_HOVER;
        pitem->state |= ewxAUI_BUTTON_STATE_PRESSED;
    }
...
void ewxAuiToolBar::RefreshOverflowState()
{
    ...
    if (overflow_rect.Contains(pt.x, pt.y))
    {
        if (::wxGetMouseState().LeftDown())
            overflow_state = ewxAUI_BUTTON_STATE_PRESSED;
        else
            overflow_state = ewxAUI_BUTTON_STATE_HOVER;
    }
...
void ewxAuiToolBar::ToggleTool(int tool_id, bool state)
{
    ewxAuiToolBarItem* tool = FindTool(tool_id);
    ...
    if (tool && (tool->kind == wxITEM_CHECK || tool->kind == wxITEM_RADIO))
    {
        if (tool->kind == wxITEM_RADIO)
        {
            ...
            if (idx >= 0 && idx < count)
            {
                for (i = idx; i < count; ++i)
                {
                    if (m_items[i].kind != wxITEM_RADIO)
                        break;
                    m_items[i].state &= ~ewxAUI_BUTTON_STATE_CHECKED;
                }
                for (i = idx; i > 0; i--)
                {
                    if (m_items[i].kind != wxITEM_RADIO)
            ...
            tool->state |= ewxAUI_BUTTON_STATE_CHECKED;
        }
...
bool ewxAuiToolBar::GetToolToggled(int tool_id) const
{
    ewxAuiToolBarItem* tool = FindTool(tool_id);
    ...
    if (tool)
    {
        ...
        return (tool->state & ewxAUI_BUTTON_STATE_CHECKED) ? true : false;
    }

    return false;
}

void ewxAuiToolBar::EnableTool(int tool_id, bool state)
{
    ewxAuiToolBarItem* tool = FindTool(tool_id);
    ...
    {
        if (state == true)
            tool->state &= ~ewxAUI_BUTTON_STATE_DISABLED;
        else
            tool->state |= ewxAUI_BUTTON_STATE_DISABLED;
    }
}

bool ewxAuiToolBar::GetToolEnabled(int tool_id) const
{
    ewxAuiToolBarItem* tool = FindTool(tool_id);

    if (tool)
        return (tool->state & ewxAUI_BUTTON_STATE_DISABLED) ? false : true;

    return false;
}

wxString ewxAuiToolBar::GetToolLabel(int tool_id) const
{
    ewxAuiToolBarItem* tool = FindTool(tool_id);
...
void ewxAuiToolBar::SetToolLabel(int tool_id, const wxString& label)
{
    ewxAuiToolBarItem* tool = FindTool(tool_id);
    if (tool)
    {
...
wxBitmap ewxAuiToolBar::GetToolBitmap(int tool_id) const
{
    ewxAuiToolBarItem* tool = FindTool(tool_id);
    wxASSERT_MSG(tool, wxT("can't find tool in toolbar item array"));
    if (!tool)
...
void ewxAuiToolBar::SetToolBitmap(int tool_id, const wxBitmap& bitmap)
{
    ewxAuiToolBarItem* tool = FindTool(tool_id);
    if (tool)
    {
...
wxString ewxAuiToolBar::GetToolShortHelp(int tool_id) const
{
    ewxAuiToolBarItem* tool = FindTool(tool_id);
    wxASSERT_MSG(tool, wxT("can't find tool in toolbar item array"));
    if (!tool)
...
void ewxAuiToolBar::SetToolShortHelp(int tool_id, const wxString& help_string)
{
    ewxAuiToolBarItem* tool = FindTool(tool_id);
    if (tool)
    {
...
wxString ewxAuiToolBar::GetToolLongHelp(int tool_id) const
{
    ewxAuiToolBarItem* tool = FindTool(tool_id);
    wxASSERT_MSG(tool, wxT("can't find tool in toolbar item array"));
    if (!tool)
...
void ewxAuiToolBar::SetToolLongHelp(int tool_id, const wxString& help_string)
{
    ewxAuiToolBarItem* tool = FindTool(tool_id);
    if (tool)
    {
...
void ewxAuiToolBar::SetCustomOverflowItems(const ewxAuiToolBarItemArray& prepend,
                                          const ewxAuiToolBarItemArray& append)
{
...
size_t ewxAuiToolBar::GetToolCount() const
{
    return m_items.size();
}

int ewxAuiToolBar::GetToolIndex(int tool_id) const
{
...
bool ewxAuiToolBar::GetToolFitsByIndex(int tool_idx) const
{
...
bool ewxAuiToolBar::GetToolFits(int tool_id) const
{
    return GetToolFitsByIndex(GetToolIndex(tool_id));
}

wxRect ewxAuiToolBar::GetToolRect(int tool_id) const
{
    ewxAuiToolBarItem* tool = FindTool(tool_id);
...
bool ewxAuiToolBar::GetToolBarFits() const
{
...
bool ewxAuiToolBar::Realize()
{
    ...
    // add gripper area
    int separator_size = m_art->GetElementSize(ewxAUI_TBART_SEPARATOR_SIZE);
    int gripper_size = m_art->GetElementSize(ewxAUI_TBART_GRIPPER_SIZE);
    ...
    if (m_style & ewxAUI_TB_OVERFLOW)
    {
        int overflow_size = m_art->GetElementSize(ewxAUI_TBART_OVERFLOW_SIZE);
    ...
    if ((m_style & ewxAUI_TB_NO_AUTORESIZE) == 0)
    {
...
int ewxAuiToolBar::GetOverflowState() const
{
    return m_overflow_state;
}

wxRect ewxAuiToolBar::GetOverflowRect() const
{
...
wxSize ewxAuiToolBar::GetLabelSize(const wxString& label)
{
    wxClientDC dc(this);
...
void ewxAuiToolBar::DoIdleUpdate()
{
        ...
        if (handler->ProcessEvent(evt))
        {
            if (evt.GetSetEnabled())
            {
                ...
                if (item.window)
                    ...
                    is_enabled = (item.state & ewxAUI_BUTTON_STATE_DISABLED) ? false : true;
                ...
                if (new_enabled != is_enabled)
                {
                    ...
                    {
                        if (new_enabled)
                            item.state &= ~ewxAUI_BUTTON_STATE_DISABLED;
                        else
                            item.state |= ewxAUI_BUTTON_STATE_DISABLED;
                    }
                    need_refresh = true;
            ...
            if (evt.GetSetChecked())
            {
                ...
                bool is_checked = (item.state & ewxAUI_BUTTON_STATE_CHECKED) ? true : false;
                bool new_checked = evt.GetChecked();

                if (new_checked != is_checked)
                {
                    if (new_checked)
                        item.state |= ewxAUI_BUTTON_STATE_CHECKED;
                    else
                        item.state &= ~ewxAUI_BUTTON_STATE_CHECKED;
...
void ewxAuiToolBar::OnSize(wxSizeEvent& WXUNUSED(evt))
{
    int x, y;
    GetClientSize(&x, &y);
...
void ewxAuiToolBar::DoSetSize(int x,
                             int y,
                             int width,
                             int height,
                             int sizeFlags)
...
void ewxAuiToolBar::OnIdle(wxIdleEvent& evt)
{
    DoIdleUpdate();
    evt.Skip();
}

void ewxAuiToolBar::OnPaint(wxPaintEvent& WXUNUSED(evt))
{
    ...
    int gripper_size = m_art->GetElementSize(ewxAUI_TBART_GRIPPER_SIZE);
    int dropdown_size = m_art->GetElementSize(ewxAUI_TBART_OVERFLOW_SIZE);
...
void ewxAuiToolBar::OnEraseBackground(wxEraseEvent& WXUNUSED(evt))
{
    // empty
}

void ewxAuiToolBar::OnLeftDown(wxMouseEvent& evt)
{
    wxRect cli_rect(wxPoint(0,0), GetClientSize());
    ...
    if (m_gripper_sizer_item)
    {
        ...
        {
            // find aui manager
            ewxAuiManager* manager = ewxAuiManager::GetManager(this);
            if (!manager)
                return;
    ...
    if (m_overflow_sizer_item)
    {
        ...
        {
            ewxAuiToolBarEvent e(wxEVT_COMMAND_AUITOOLBAR_OVERFLOW_CLICK, -1);
            ...
            {
                size_t i, count;
                ewxAuiToolBarItemArray overflow_items;
    ...
    if (m_action_item)
    {
        if (m_action_item->state & ewxAUI_BUTTON_STATE_DISABLED)
        {
            m_action_pos = wxPoint(-1,-1);
            m_action_item = NULL;
            return;
        ...
        ewxAuiToolBarEvent e(wxEVT_COMMAND_AUITOOLBAR_TOOL_DROPDOWN, m_action_item->id);
        e.SetEventObject(this);
...
void ewxAuiToolBar::OnLeftUp(wxMouseEvent& evt)
{
    SetPressedItem(NULL);

    ewxAuiToolBarItem* hit_item = FindToolByPosition(evt.GetX(), evt.GetY());
    if (hit_item && !(hit_item->state & ewxAUI_BUTTON_STATE_DISABLED))
    {
    ...
    else
    {
        ...
        if (m_action_item && hit_item == m_action_item)
        {
            ...
            if (hit_item->kind == wxITEM_CHECK || hit_item->kind == wxITEM_RADIO)
            {
                ...
                if (m_action_item->state & ewxAUI_BUTTON_STATE_CHECKED)
                    toggle = false;
                else
                    toggle = true;
...
void ewxAuiToolBar::OnRightDown(wxMouseEvent& evt)
{
    wxRect cli_rect(wxPoint(0,0), GetClientSize());
...
void ewxAuiToolBar::OnRightUp(wxMouseEvent& evt)
{
    ewxAuiToolBarItem* hit_item;
    hit_item = FindToolByPosition(evt.GetX(), evt.GetY());
    ...
    if (m_action_item && hit_item == m_action_item)
    {
        ewxAuiToolBarEvent e(wxEVT_COMMAND_AUITOOLBAR_RIGHT_CLICK, m_action_item->id);
        e.SetEventObject(this);
        e.SetToolId(m_action_item->id);
    ...
    else
    {
        ...
        ewxAuiToolBarEvent e(wxEVT_COMMAND_AUITOOLBAR_RIGHT_CLICK, -1);
        e.SetEventObject(this);
        e.SetToolId(-1);
        e.SetClickPoint(m_action_pos);
        ProcessEvent(e);
...
void ewxAuiToolBar::OnMiddleDown(wxMouseEvent& evt)
{
    wxRect cli_rect(wxPoint(0,0), GetClientSize());
...
void ewxAuiToolBar::OnMiddleUp(wxMouseEvent& evt)
{
    ewxAuiToolBarItem* hit_item;
    hit_item = FindToolByPosition(evt.GetX(), evt.GetY());
    ...
    if (m_action_item && hit_item == m_action_item)
    {
        ...
        {
            ewxAuiToolBarEvent e(wxEVT_COMMAND_AUITOOLBAR_MIDDLE_CLICK, m_action_item->id);
            e.SetEventObject(this);
            e.SetToolId(m_action_item->id);
            e.SetClickPoint(m_action_pos);
            ProcessEvent(e);
...
void ewxAuiToolBar::OnMotion(wxMouseEvent& evt)
{
    // start a drag event
    if (!m_dragging &&
        m_action_item != NULL &&
        ...
        abs(evt.m_x - m_action_pos.x) + abs(evt.m_y - m_action_pos.y) > 5)
    {
        ...
        ewxAuiToolBarEvent e(wxEVT_COMMAND_AUITOOLBAR_BEGIN_DRAG, GetId());
        e.SetEventObject(this);
        e.SetToolId(m_action_item->id);
        ProcessEvent(e);
        DoIdleUpdate();
    ...
    if (hit_item)
    {
        if (!(hit_item->state & ewxAUI_BUTTON_STATE_DISABLED))
            SetHoverItem(hit_item);
        else
            SetHoverItem(NULL);
    }
    ...
    // figure out tooltips
    ewxAuiToolBarItem* packing_hit_item;
...
void ewxAuiToolBar::OnLeaveWindow(wxMouseEvent& WXUNUSED(evt))
{
    RefreshOverflowState();
    SetHoverItem(NULL);
    SetPressedItem(NULL);
...
void ewxAuiToolBar::OnSetCursor(wxSetCursorEvent& evt)
{
