...
IMPLEMENT_CLASS(ewxAuiFloatingFrame, ewxAuiFloatingFrameBaseClass)

ewxAuiFloatingFrame::ewxAuiFloatingFrame(wxWindow* parent,
                ...
                long style /*=wxRESIZE_BORDER | wxSYSTEM_MENU | wxCAPTION |
                              ...
                              wxCLIP_CHILDREN
                           */)
                : ewxAuiFloatingFrameBaseClass(parent, id, wxEmptyString,
                        pane.floating_pos, pane.floating_size,
                        style | 
                        (pane.HasCloseButton()?wxCLOSE_BOX:0) |
                        (pane.HasMaximizeButton()?wxMAXIMIZE_BOX:0) |
(pane.IsAlwaysFixed()?0:wxRESIZE_BORDER)
                        )
{
...
ewxAuiFloatingFrame::~ewxAuiFloatingFrame()
{
...
void ewxAuiFloatingFrame::SetPaneWindow(const ewxAuiPaneInfo& pane)
{
    m_pane_window = pane.window;
    m_pane_window->Reparent(this);

    ewxAuiPaneInfo contained_pane = pane;
        ...
        else
    {
        ...
        if (pane.HasGripper())
        {
            if (pane.HasGripperTop())
                size.y += m_owner_mgr->m_art->GetMetric(ewxAUI_DOCKART_GRIPPER_SIZE);
            else
                size.x += m_owner_mgr->m_art->GetMetric(ewxAUI_DOCKART_GRIPPER_SIZE);
        }
...
ewxAuiManager* ewxAuiFloatingFrame::GetOwnerManager() const
{
    return m_owner_mgr;
}


void ewxAuiFloatingFrame::OnSize(wxSizeEvent& event)
{
    m_owner_mgr->OnFloatingPaneResized(m_pane_window, event.GetSize());
}

void ewxAuiFloatingFrame::OnClose(wxCloseEvent& evt)
{
...
void ewxAuiFloatingFrame::OnMoveEvent(wxMoveEvent& event)
{
    if (!m_solid_drag)
    {
...
void ewxAuiFloatingFrame::OnIdle(wxIdleEvent& event)
{
    if (m_moving)
    {
        if (!isMouseDown())
...
void ewxAuiFloatingFrame::OnMoveStart()
{
    // notify the owner manager that the pane has started to move
    m_owner_mgr->OnFloatingPaneMoveStart(m_pane_window);
}

void ewxAuiFloatingFrame::OnMoving(const wxRect& WXUNUSED(window_rect), wxDirection dir)
{
...
void ewxAuiFloatingFrame::OnMoveFinished()
{
    // notify the owner manager that the pane has finished moving
    m_owner_mgr->OnFloatingPaneMoved(m_pane_window, m_lastDirection);
}

void ewxAuiFloatingFrame::OnActivate(wxActivateEvent& event)
{
    if (event.GetActive())
    {
...
bool ewxAuiFloatingFrame::isMouseDown()
{
    return wxGetMouseState().LeftDown();
}


BEGIN_EVENT_TABLE(ewxAuiFloatingFrame, ewxAuiFloatingFrameBaseClass)
    EVT_SIZE(ewxAuiFloatingFrame::OnSize)
    EVT_MOVE(ewxAuiFloatingFrame::OnMoveEvent)
    EVT_MOVING(ewxAuiFloatingFrame::OnMoveEvent)
    EVT_CLOSE(ewxAuiFloatingFrame::OnClose)
    EVT_IDLE(ewxAuiFloatingFrame::OnIdle)
    EVT_ACTIVATE(ewxAuiFloatingFrame::OnActivate)
END_EVENT_TABLE()
