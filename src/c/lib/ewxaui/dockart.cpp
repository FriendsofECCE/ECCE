...
// -- ewxAuiDefaultDockArt class implementation --

// ewxAuiDefaultDockArt is an art provider class which does all of the drawing for
// ewxAuiManager.  This allows the library caller to customize the dock art
...
// from the ewxAuiDockArt base class). The active dock art class can be set via
// ewxAuiManager::SetDockArt()


// ewxAuiBlendColour is used by ewxAuiStepColour
...
// ewxAuiStepColour() it a utility function that simply darkens
// or lightens a color, based on the specified percentage
// ialpha of 0 would be completely black, 100 completely white
// an ialpha of 100 returns the same colour
wxColor ewxAuiStepColour(const wxColor& c, int ialpha)
{
    if (ialpha == 100)
        return c;
    ...
    r = ewxAuiBlendColour(r, bg, alpha);
    g = ewxAuiBlendColour(g, bg, alpha);
    b = ewxAuiBlendColour(b, bg, alpha);

    return wxColour(r, g, b);
}


wxColor ewxAuiLightContrastColour(const wxColour& c)
{
    int amount = 120;
    ...
    return ewxAuiStepColour(c, amount);
}

// ewxAuiBitmapFromBits() is a utility function that creates a
                                  ...
                                  int direction)
{
    ...
    if (direction == ewxAUI_GRADIENT_VERTICAL)
...
ewxAuiDefaultDockArt::ewxAuiDefaultDockArt()
{
#ifdef __WXMAC__
    ...
    wxColor darker1_colour = ewxAuiStepColour(base_colour, 85);
    wxColor darker2_colour = ewxAuiStepColour(base_colour, 75);
    wxColor darker3_colour = ewxAuiStepColour(base_colour, 60);
    wxColor darker4_colour = ewxAuiStepColour(base_colour, 50);
    wxColor darker5_colour = ewxAuiStepColour(base_colour, 40);

    m_active_caption_colour = wxSystemSettings::GetColour(wxSYS_COLOUR_HIGHLIGHT);
    m_active_caption_gradient_colour = ewxAuiLightContrastColour(wxSystemSettings::GetColour(wxSYS_COLOUR_HIGHLIGHT));
    ...
    m_inactive_caption_gradient_colour = ewxAuiStepColour(base_colour, 97);
...
#ifdef __WXMAC__
    m_inactive_close_bitmap = ewxAuiBitmapFromBits(close_bits, 16, 16, *wxWHITE);
    m_active_close_bitmap = ewxAuiBitmapFromBits(close_bits, 16, 16, *wxWHITE );
#else
    m_inactive_close_bitmap = ewxAuiBitmapFromBits(close_bits, 16, 16, m_inactive_caption_text_colour);
    m_active_close_bitmap = ewxAuiBitmapFromBits(close_bits, 16, 16, m_active_caption_text_colour);
#endif

#ifdef __WXMAC__
    m_inactive_maximize_bitmap = ewxAuiBitmapFromBits(maximize_bits, 16, 16, *wxWHITE);
    m_active_maximize_bitmap = ewxAuiBitmapFromBits(maximize_bits, 16, 16, *wxWHITE );
#else
    m_inactive_maximize_bitmap = ewxAuiBitmapFromBits(maximize_bits, 16, 16, m_inactive_caption_text_colour);
    m_active_maximize_bitmap = ewxAuiBitmapFromBits(maximize_bits, 16, 16, m_active_caption_text_colour);
#endif

#ifdef __WXMAC__
    m_inactive_restore_bitmap = ewxAuiBitmapFromBits(restore_bits, 16, 16, *wxWHITE);
    m_active_restore_bitmap = ewxAuiBitmapFromBits(restore_bits, 16, 16, *wxWHITE );
#else
    m_inactive_restore_bitmap = ewxAuiBitmapFromBits(restore_bits, 16, 16, m_inactive_caption_text_colour);
    m_active_restore_bitmap = ewxAuiBitmapFromBits(restore_bits, 16, 16, m_active_caption_text_colour);
#endif

    m_inactive_pin_bitmap = ewxAuiBitmapFromBits(pin_bits, 16, 16, m_inactive_caption_text_colour);
    m_active_pin_bitmap = ewxAuiBitmapFromBits(pin_bits, 16, 16, m_active_caption_text_colour);
    ...
    m_button_size = 14;
    m_gripper_size = 9;
    m_gradient_type = ewxAUI_GRADIENT_VERTICAL;
...
int ewxAuiDefaultDockArt::GetMetric(int id)
{
    switch (id)
    {
        case ewxAUI_DOCKART_SASH_SIZE:          return m_sash_size;
        case ewxAUI_DOCKART_CAPTION_SIZE:       return m_caption_size;
        case ewxAUI_DOCKART_GRIPPER_SIZE:       return m_gripper_size;
        case ewxAUI_DOCKART_PANE_BORDER_SIZE:   return m_border_size;
        case ewxAUI_DOCKART_PANE_BUTTON_SIZE:   return m_button_size;
        case ewxAUI_DOCKART_GRADIENT_TYPE:      return m_gradient_type;
...
void ewxAuiDefaultDockArt::SetMetric(int id, int new_val)
{
    switch (id)
    {
        case ewxAUI_DOCKART_SASH_SIZE:          m_sash_size = new_val; break;
        case ewxAUI_DOCKART_CAPTION_SIZE:       m_caption_size = new_val; break;
        case ewxAUI_DOCKART_GRIPPER_SIZE:       m_gripper_size = new_val; break;
        case ewxAUI_DOCKART_PANE_BORDER_SIZE:   m_border_size = new_val; break;
        case ewxAUI_DOCKART_PANE_BUTTON_SIZE:   m_button_size = new_val; break;
        case ewxAUI_DOCKART_GRADIENT_TYPE:      m_gradient_type = new_val; break;
        default: wxFAIL_MSG(wxT("Invalid Metric Ordinal")); break;
    }
}

wxColour ewxAuiDefaultDockArt::GetColour(int id)
{
    switch (id)
    {
        case ewxAUI_DOCKART_BACKGROUND_COLOUR:                return m_background_brush.GetColour();
        case ewxAUI_DOCKART_SASH_COLOUR:                      return m_sash_brush.GetColour();
        case ewxAUI_DOCKART_INACTIVE_CAPTION_COLOUR:          return m_inactive_caption_colour;
        case ewxAUI_DOCKART_INACTIVE_CAPTION_GRADIENT_COLOUR: return m_inactive_caption_gradient_colour;
        case ewxAUI_DOCKART_INACTIVE_CAPTION_TEXT_COLOUR:     return m_inactive_caption_text_colour;
        case ewxAUI_DOCKART_ACTIVE_CAPTION_COLOUR:            return m_active_caption_colour;
        case ewxAUI_DOCKART_ACTIVE_CAPTION_GRADIENT_COLOUR:   return m_active_caption_gradient_colour;
        case ewxAUI_DOCKART_ACTIVE_CAPTION_TEXT_COLOUR:       return m_active_caption_text_colour;
        case ewxAUI_DOCKART_BORDER_COLOUR:                    return m_border_pen.GetColour();
        case ewxAUI_DOCKART_GRIPPER_COLOUR:                   return m_gripper_brush.GetColour();
        default: wxFAIL_MSG(wxT("Invalid Metric Ordinal")); break;
    }
...
void ewxAuiDefaultDockArt::SetColour(int id, const wxColor& colour)
{
    switch (id)
    {
        case ewxAUI_DOCKART_BACKGROUND_COLOUR:                m_background_brush.SetColour(colour); break;
        case ewxAUI_DOCKART_SASH_COLOUR:                      m_sash_brush.SetColour(colour); break;
        case ewxAUI_DOCKART_INACTIVE_CAPTION_COLOUR:          m_inactive_caption_colour = colour; break;
        case ewxAUI_DOCKART_INACTIVE_CAPTION_GRADIENT_COLOUR: m_inactive_caption_gradient_colour = colour; break;
        case ewxAUI_DOCKART_INACTIVE_CAPTION_TEXT_COLOUR:     m_inactive_caption_text_colour = colour; break;
        case ewxAUI_DOCKART_ACTIVE_CAPTION_COLOUR:            m_active_caption_colour = colour; break;
        case ewxAUI_DOCKART_ACTIVE_CAPTION_GRADIENT_COLOUR:   m_active_caption_gradient_colour = colour; break;
        case ewxAUI_DOCKART_ACTIVE_CAPTION_TEXT_COLOUR:       m_active_caption_text_colour = colour; break;
        case ewxAUI_DOCKART_BORDER_COLOUR:                    m_border_pen.SetColour(colour); break;
        case ewxAUI_DOCKART_GRIPPER_COLOUR:
            m_gripper_brush.SetColour(colour);
            m_gripper_pen1.SetColour(ewxAuiStepColour(colour, 40));
            m_gripper_pen2.SetColour(ewxAuiStepColour(colour, 60));
            break;
        default: wxFAIL_MSG(wxT("Invalid Metric Ordinal")); break;
    }
}

void ewxAuiDefaultDockArt::SetFont(int id, const wxFont& font)
{
    if (id == ewxAUI_DOCKART_CAPTION_FONT)
        m_caption_font = font;
}

wxFont ewxAuiDefaultDockArt::GetFont(int id)
{
    if (id == ewxAUI_DOCKART_CAPTION_FONT)
...
void ewxAuiDefaultDockArt::DrawSash(wxDC& dc, wxWindow *window, int orientation, const wxRect& rect)
{
#if defined(__WXMAC__)
...
void ewxAuiDefaultDockArt::DrawBackground(wxDC& dc, wxWindow *WXUNUSED(window), int, const wxRect& rect)
{
    dc.SetPen(*wxTRANSPARENT_PEN);
#ifdef __WXMAC__
...
void ewxAuiDefaultDockArt::DrawBorder(wxDC& dc, wxWindow *WXUNUSED(window), const wxRect& _rect,
                                  ewxAuiPaneInfo& pane)
{
    ...
    wxRect rect = _rect;
    int i, border_width = GetMetric(ewxAUI_DOCKART_PANE_BORDER_SIZE);
...
void ewxAuiDefaultDockArt::DrawCaptionBackground(wxDC& dc, const wxRect& rect, bool active)
{
    if (m_gradient_type == ewxAUI_GRADIENT_NONE)
    {
        if (active)
...
void ewxAuiDefaultDockArt::DrawCaption(wxDC& dc, wxWindow *WXUNUSED(window),
                                   const wxString& text,
                                   const wxRect& rect,
                                   ewxAuiPaneInfo& pane)
{
    ...
    DrawCaptionBackground(dc, rect,
                          (pane.state & ewxAuiPaneInfo::optionActive)?true:false);
    ...
    wxString draw_text = ewxAuiChopText(dc, text, clip_rect.width);

    dc.SetClippingRegion(clip_rect);
    dc.DrawText(draw_text, rect.x+3+rectMult*m_button_size, rect.y+(rect.height/2)-(h/2)-1);
    dc.DestroyClippingRegion();
}

void ewxAuiDefaultDockArt::DrawGripper(wxDC& dc, wxWindow *WXUNUSED(window),
                                   const wxRect& rect,
                                   ewxAuiPaneInfo& pane)
{
    dc.SetPen(*wxTRANSPARENT_PEN);
...
void ewxAuiDefaultDockArt::DrawPaneButton(wxDC& dc, wxWindow *WXUNUSED(window),
                                      int button,
                                      int button_state,
                                      const wxRect& _rect,
                                      ewxAuiPaneInfo& pane)
{
    ...
    switch (button)
    {
        ...
        case ewxAUI_BUTTON_PIN:
            ...
            break;
        case ewxAUI_BUTTON_MAXIMIZE_RESTORE:
            if (pane.IsMaximized())
            {
                if (pane.state & ewxAuiPaneInfo::optionActive)
        ...
        button_state == ewxAUI_BUTTON_STATE_PRESSED)
    {
        if (pane.state & ewxAuiPaneInfo::optionActive)
        {
            dc.SetBrush(wxBrush(ewxAuiStepColour(m_active_caption_colour, 120)));
            dc.SetPen(wxPen(ewxAuiStepColour(m_active_caption_colour, 70)));
        }
         else
        {
            dc.SetBrush(wxBrush(ewxAuiStepColour(m_inactive_caption_colour, 120)));
            dc.SetPen(wxPen(ewxAuiStepColour(m_inactive_caption_colour, 70)));
        }
