...
#include "inv/ChemKit/IsoLib.hpp"
...
////////////////////////////////////////////////////////////////////////
{ 
    SO_NODE_CONSTRUCTOR(ChemIso);

    SO_NODE_ADD_FIELD(threshold, (0.0));
    SO_NODE_ADD_FIELD(generateNormals, (TRUE));
    SO_NODE_ADD_FIELD(flipNormals, (FALSE));
    ...
    children->append(triStrip);

    SoVertexProperty *vp = new SoVertexProperty;
    vp->ref();
    triStrip->vertexProperty.setValue(vp);

    // Initialize private variables.
    first = TRUE;
    lastGenerateNormals = TRUE;
    lastFlipNormals = FALSE;
    lastThreshold = 0.0;
    lastDataVar = 0;
    lastColorVar = 0;
...
////////////////////////////////////////////////////////////////////////
{
    ...
    // Deleting triStrip is not necessary since it was unrefed by the
    // the remove.
...
void
ChemIso::doAction(SoAction *action)
//
////////////////////////////////////////////////////////////////////////
{
    regenerate(FALSE);
...
void
ChemIso::GLRender(SoGLRenderAction *action)
//
////////////////////////////////////////////////////////////////////////
{
    if (!shouldGLRender(action)) return;
    ChemIso::doAction(action);
}
...
void
ChemIso::pick(SoPickAction *action)
//
////////////////////////////////////////////////////////////////////////
{
    ChemIso::doAction(action);
...
//
// Description:
//    Picks polys representing a triangleStrip set.
//
// Use: extender
   
void
ChemIso::rayPick(SoRayPickAction *action)
//
////////////////////////////////////////////////////////////////////////
{
...
void
ChemIso::callback(SoCallbackAction *action)
//
////////////////////////////////////////////////////////////////////////
{
    ChemIso::doAction(action);
...
void
ChemIso::write(SoWriteAction *action)
//
////////////////////////////////////////////////////////////////////////
{
    SoNode::write(action);
...
void
ChemIso::getBoundingBox(SoGetBoundingBoxAction *action)
//
////////////////////////////////////////////////////////////////////////
{
    // From Hansres:
...
void
ChemIso::search(SoSearchAction *action)
//
////////////////////////////////////////////////////////////////////////
{
...
void
ChemIso::handleEvent(SoHandleEventAction *action)
//
////////////////////////////////////////////////////////////////////////
{
    ChemIso::doAction(action);
...
void
ChemIso::getMatrix(SoGetMatrixAction *action)
//
////////////////////////////////////////////////////////////////////////
{
...
void
ChemIso::generatePrimitives(SoAction *action)
//
////////////////////////////////////////////////////////////////////////
{
    regenerate(FALSE);
    ((ChemIso *)triStrip)->generatePrimitives(action);
}
 
////////////////////////////////////////////////////////////////////////
//
// Description:
//    Computes bounding box of polys representing a triangleStrip set.
//
// Use: protected
   
void
ChemIso::computeBBox(SoAction *action, SbBox3f &box, SbVec3f &center)
//
////////////////////////////////////////////////////////////////////////
{
fprintf(stderr, "In ChemIso::computeBBox.  THIS SHOULD NEVER HAPPEN\n\n");

    regenerate(FALSE);
    ((SoShape *)triStrip)->computeBBox(action, box, center);
}

////////////////////////////////////////////////////////////////////////
//
// Description:
//    Recalculate the isosurface if values have changed
//
// Use: private
    
void
ChemIso::regenerate(SbBool forceIt)
//  
////////////////////////////////////////////////////////////////////////
{ 
    ...
    if (dataLat == NULL) {
#ifdef DEBUG
        SoDebug::post("ChemIso::regenerate",
    ...
    if (first) {
        ...
        if (!forceIt) {
            if ((lastGenerateNormals == generateNormals.getValue()) &&
                 (lastFlipNormals == flipNormals.getValue()) &&
    ...
    lastGenerateNormals = generateNormals.getValue();
    lastFlipNormals = flipNormals.getValue();
    lastThreshold = threshold.getValue();
    lastDataVar = dataVar.getValue();
    lastColorVar = colorVar.getValue();
    ...
    triStrip->coordIndex.deleteValues(0,-1);

    vp->vertex.deleteValues(0,-1);   
    vp->normal.deleteValues(0,-1);
