...
public:
    ...
    // Compute the 2-norm (i.e. sqrt( sum_i(y_i*conjugate(y_i)) ) )
    // It is assumed that data is ordered with respect to X
    double Get2Norm(int start_index =0, int end_index = -1) const;
    ...
    // Two-sided Fast Fourier Transform the data, forward = true for forward
    //   transform, forward = false for inverse transform.
    // Note that the data size will be expanded to the smallest 2^n size that
    //   fully contains the data by appending zeros if necessary.
    // Since there are typically real and imaginary parts GetYiData() is now valid
    wxPlotData FFT( bool forward ) const;
    // Returns a two-sided power spectrum of the FFT of the data.
    //   Pxx = [ FFT * (FFT*) ] / N^2; N = length of original wxPlotData
    wxPlotData PowerSpectrum() const;
    // Return the normalized one-sided power spectrum of the data
    // Check W.Press, ... "Numerical Recipes in C", Chapter 12 for the definition
    wxPlotData OneSidedPowerSpectrum() const;
    ...
    wxPlotData FFTHiPassFilter( double fc, wxPlotData::FFTFilter_Type filter, double n = 5 ) const;
    wxPlotData FFTLoPassFilter( double fc, wxPlotData::FFTFilter_Type filter, double n = 5 ) const;
    ...
    wxPlotData FFTNotchFilter( double lo, double hi, wxPlotData::FFTFilter_Type filter, double n = 5 ) const;
    wxPlotData FFTBandPassFilter( double lo, double hi, wxPlotData::FFTFilter_Type filter, double n = 5 ) const;

    // After FFT this curve, apply the filter (mult y points at each x) and transform back
    wxPlotData FFTCustomFilter( const wxPlotFunction &func ) const;
