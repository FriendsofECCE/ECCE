...
#ifndef _EWX_AUIBAR_H_
#define _EWX_AUIBAR_H_
...
enum ewxAuiToolBarStyle
{
    ewxAUI_TB_TEXT          = 1 << 0,
    ewxAUI_TB_NO_TOOLTIPS   = 1 << 1,
    ewxAUI_TB_NO_AUTORESIZE = 1 << 2,
    ewxAUI_TB_GRIPPER       = 1 << 3,
    ewxAUI_TB_OVERFLOW      = 1 << 4,
    ewxAUI_TB_VERTICAL      = 1 << 5,
    ewxAUI_TB_HORZ_LAYOUT   = 1 << 6,
    ewxAUI_TB_HORZ_TEXT     = (ewxAUI_TB_HORZ_LAYOUT | ewxAUI_TB_TEXT),
    ewxAUI_TB_DEFAULT_STYLE = 0
};

enum ewxAuiToolBarArtSetting
{
    ewxAUI_TBART_SEPARATOR_SIZE = 0,
    ewxAUI_TBART_GRIPPER_SIZE = 1,
    ewxAUI_TBART_OVERFLOW_SIZE = 2
};

enum ewxAuiToolBarToolTextOrientation
{
    ewxAUI_TBTOOL_TEXT_LEFT = 0,     // unused/unimplemented
    ewxAUI_TBTOOL_TEXT_RIGHT = 1,
    ewxAUI_TBTOOL_TEXT_TOP = 2,      // unused/unimplemented
    ewxAUI_TBTOOL_TEXT_BOTTOM = 3
};


// aui toolbar event class

class ewxAuiToolBarEvent : public wxNotifyEvent
{
public:
    ewxAuiToolBarEvent(wxEventType command_type = wxEVT_NULL,
                      int win_id = 0)
...
#ifndef SWIG
    ewxAuiToolBarEvent(const ewxAuiToolBarEvent& c) : wxNotifyEvent(c)
    {
...
#endif
    wxEvent *Clone() const { return new ewxAuiToolBarEvent(*this); }
...
#ifndef SWIG
private:
    DECLARE_DYNAMIC_CLASS_NO_ASSIGN(ewxAuiToolBarEvent)
#endif
};


class ewxAuiToolBarItem
{
    friend class ewxAuiToolBar;
...
public:
    ...
    ewxAuiToolBarItem(const ewxAuiToolBarItem& c)
    {
        Assign(c);
    }

    ewxAuiToolBarItem& operator=(const ewxAuiToolBarItem& c)
    {
        Assign(c);
        return *this;
    }

    void Assign(const ewxAuiToolBarItem& c)
    {
        window = c.window;
        label = c.label;
        bitmap = c.bitmap;
...
#ifndef SWIG
WX_DECLARE_OBJARRAY(ewxAuiToolBarItem, ewxAuiToolBarItemArray);
#endif
...
class ewxAuiToolBarArt
{
public:

    ewxAuiToolBarArt() { }
    virtual ~ewxAuiToolBarArt() { }

    virtual ewxAuiToolBarArt* Clone() = 0;
    ...
    virtual wxSize GetLabelSize(
                         wxDC& dc,
                         wxWindow* wnd,
                         const ewxAuiToolBarItem& item) = 0;
    ...
    virtual int ShowDropDown(
                         wxWindow* wnd,
                         const ewxAuiToolBarItemArray& items) = 0;
};



class ewxAuiDefaultToolBarArt : public ewxAuiToolBarArt
{

public:

    ewxAuiDefaultToolBarArt();
    virtual ~ewxAuiDefaultToolBarArt();

    virtual ewxAuiToolBarArt* Clone();
...
class ewxAuiToolBar : public wxControl
{
public:

    ewxAuiToolBar(wxWindow* parent,
                 wxWindowID id = -1,
                 const wxPoint& position = wxDefaultPosition,
                 const wxSize& size = wxDefaultSize,
                 long style = ewxAUI_TB_DEFAULT_STYLE);
    ~ewxAuiToolBar();

    void SetWindowStyleFlag(long style);
    long GetWindowStyleFlag() const;

    void SetArtProvider(ewxAuiToolBarArt* art);
    ewxAuiToolBarArt* GetArtProvider() const;
    ...
    ewxAuiToolBarItem* FindToolByPosition(wxCoord x, wxCoord y) const;
    ewxAuiToolBarItem* FindToolByIndex(int idx) const;
    ewxAuiToolBarItem* FindTool(int tool_id) const;
    ...
    void SetCustomOverflowItems(const ewxAuiToolBarItemArray& prepend,
                                const ewxAuiToolBarItemArray& append);

protected:

    virtual void OnCustomRender(wxDC& WXUNUSED(dc),
                                const ewxAuiToolBarItem& WXUNUSED(item),
...
protected:
    ...
    void SetHoverItem(ewxAuiToolBarItem* item);
    void SetPressedItem(ewxAuiToolBarItem* item);
    void RefreshOverflowState();

    int GetOverflowState() const;
    wxRect GetOverflowRect() const;
    wxSize GetLabelSize(const wxString& label);
    ewxAuiToolBarItem* FindToolByPositionWithPacking(wxCoord x, wxCoord y) const;
...
protected:
    ...
    ewxAuiToolBarItemArray m_items;      // array of toolbar items
    ewxAuiToolBarArt* m_art;             // art provider
    wxBoxSizer* m_sizer;                // main sizer for toolbar
    ewxAuiToolBarItem* m_action_item;    // item that's being acted upon (pressed)
    ewxAuiToolBarItem* m_tip_item;       // item that has its tooltip shown
    ...
    ewxAuiToolBarItemArray m_custom_overflow_prepend;
    ewxAuiToolBarItemArray m_custom_overflow_append;
...
BEGIN_DECLARE_EVENT_TYPES()
    DECLARE_EVENT_TYPE(wxEVT_COMMAND_AUITOOLBAR_TOOL_DROPDOWN, 0)
    DECLARE_EVENT_TYPE(wxEVT_COMMAND_AUITOOLBAR_OVERFLOW_CLICK, 0)
    DECLARE_EVENT_TYPE(wxEVT_COMMAND_AUITOOLBAR_RIGHT_CLICK, 0)
    DECLARE_EVENT_TYPE(wxEVT_COMMAND_AUITOOLBAR_MIDDLE_CLICK, 0)
    DECLARE_EVENT_TYPE(wxEVT_COMMAND_AUITOOLBAR_BEGIN_DRAG, 0)
END_DECLARE_EVENT_TYPES()

typedef void (wxEvtHandler::*ewxAuiToolBarEventFunction)(ewxAuiToolBarEvent&);

#define ewxAuiToolBarEventHandler(func) \
    (wxObjectEventFunction)(wxEventFunction)wxStaticCastEvent(ewxAuiToolBarEventFunction, &func)

#define EVT_EWXAUITOOLBAR_TOOL_DROPDOWN(winid, fn) \
    wx__DECLARE_EVT1(wxEVT_COMMAND_AUITOOLBAR_TOOL_DROPDOWN, winid, ewxAuiToolBarEventHandler(fn))
#define EVT_EWXAUITOOLBAR_OVERFLOW_CLICK(winid, fn) \
    wx__DECLARE_EVT1(wxEVT_COMMAND_AUITOOLBAR_OVERFLOW_CLICK, winid, ewxAuiToolBarEventHandler(fn))
#define EVT_EWXAUITOOLBAR_RIGHT_CLICK(winid, fn) \
    wx__DECLARE_EVT1(wxEVT_COMMAND_AUITOOLBAR_RIGHT_CLICK, winid, ewxAuiToolBarEventHandler(fn))
#define EVT_EWXAUITOOLBAR_MIDDLE_CLICK(winid, fn) \
    wx__DECLARE_EVT1(wxEVT_COMMAND_AUITOOLBAR_MIDDLE_CLICK, winid, ewxAuiToolBarEventHandler(fn))
#define EVT_EWXAUITOOLBAR_BEGIN_DRAG(winid, fn) \
    wx__DECLARE_EVT1(wxEVT_COMMAND_AUITOOLBAR_BEGIN_DRAG, winid, ewxAuiToolBarEventHandler(fn))
...
%pythoncode {
    EVT_EWXAUITOOLBAR_TOOL_DROPDOWN = wx.PyEventBinder( wxEVT_COMMAND_AUITOOLBAR_TOOL_DROPDOWN, 1 )
    EVT_EWXAUITOOLBAR_OVERFLOW_CLICK = wx.PyEventBinder( wxEVT_COMMAND_AUITOOLBAR_OVERFLOW_CLICK, 1 )
    EVT_EWXAUITOOLBAR_RIGHT_CLICK = wx.PyEventBinder( wxEVT_COMMAND_AUITOOLBAR_RIGHT_CLICK, 1 )
    EVT_EWXAUITOOLBAR_MIDDLE_CLICK = wx.PyEventBinder( wxEVT_COMMAND_AUITOOLBAR_MIDDLE_CLICK, 1 )
    EVT_EWXAUITOOLBAR_BEGIN_DRAG = wx.PyEventBinder( wxEVT_COMMAND_AUITOOLBAR_BEGIN_DRAG, 1 )
}
#endif  // SWIG
