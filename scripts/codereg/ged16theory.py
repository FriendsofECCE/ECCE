...
class Ged16TheoryFrame(EcceFrame):
    def __init__(self, parent, title, app, helpURL=""):
        EcceFrame.__init__(self, parent, title)
        panel = Ged16TheoryPanel(self, helpURL)
        self.Finalize()
        
class Ged16TheoryPanel(EccePanel):
...
#                              "RevTPSS",
        ...
        # none of which is theory-specific. It was simply never offered,
        # because the box lived inside the DFT and MP branches.
        if EcceGlobals.Category == "SCF":
            self.addSolvationBox()
    ...
    def addSolvationBox(self):
        """Build the SCRF solvation box.

        This was duplicated verbatim in the DFT and MP branches and absent
        everywhere else -- so a plain HF job had no solvation UI at all,
        even though Gaussian's SCRF works fine with HF and ai.gauss16's
        SCRFOptions is not theory-specific (issue #98). One copy, called
        from each branch that should offer it.
        """
    ...
    def CheckDependency(self):
        ...
        # SCRF solvation -- CAO
        # Must list exactly the categories that call addSolvationBox()
        # above: naming one that does not build the widgets dereferences
...
frame = Ged16TheoryFrame(None,
                         title = "ECCE Gaussian-16 Editor: Theory Details",
                         app = app,
                         helpURL = "")
