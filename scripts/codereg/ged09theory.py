...
#       self.panelSizer.Add(genopsSizer)
        ...
        if EcceGlobals.Category == "DFT":
            ...
            #  ALPHABETICAL, "None" first; see ged16theory.py for the
            #  reasoning. The defaults below are looked up by NAME so the
            #  ordering can change without silently changing them.
            xcFuncChoice = [
                            "None",
                            "APF (hybrid)",
                            "APFD (hybrid)",
...
#                              "RevTPSS",
    ...
    def addSolvationBox(self):
        ...
        Duplicated verbatim in the DFT and MP branches and absent
        everywhere else, so a plain HF job had no solvation UI at all --
        even though Gaussian's SCRF works fine with HF and ai.gauss09's
        SCRFOptions is not theory-specific (issue #98).
        """
