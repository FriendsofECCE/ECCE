...
sub wrGames{
  ...
  foreach $atom (keys %gbs)
  {
    ...
    print "$en\n";
    #
    # deal with all pointers to orbitals
    #
    my $orbPtr;
    foreach $orbPtr (@orbitalList)
    {
      ...
      #
      # $contcntr counts number of rows
      #
      my $contcntr = $#contractionSet + 1;
      #
      # $columncntr counts number of columns in first row
      #
      my $columnctr = $#{$contractionSet[0]};
      my $i = 0;
      for ( $i=0; $i < $columnctr; $i++ ) 
      {
        my $shell = ($orbitalType eq "SP") ? "L" : $orbitalType;
        printf "%2s%4d\n", $shell, $contcntr;
        my $linectr = 0;
        my $rowPtr;
        foreach $rowPtr (@contractionSet)
        {
          ...
          printf "%3d", $linectr;
          printf "%19.8f", $exponent;
          printf "%17.8f", $coefficients[$i];
          printf "%17.8f", $coefficients[$i+1] if $shell eq "L";
          print "\n";
        }
        ++$i if $shell eq "L";
      }
